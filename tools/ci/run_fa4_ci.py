...
def prepare_overlay(
    ...
    # `sync` so the ext3 image behind --overlay is flushed before this session tears down; see
    # wait_for_overlay_release() for why the hand-off between sessions needs care.
...
#
# Every apptainer session mounts the disk-backed --overlay image through fuse2fs + fuse-overlayfs.
# `apptainer exec` returns as soon as its mounts are unmounted, but the fuse2fs child that owns the
# ext3 image may still be flushing metadata for a moment. A session opened during that window sees
# the image WITHOUT the whiteouts / opaque markers the previous session wrote, i.e. the SIF-baked
# packages that prepare_overlay() deleted are visible again next to the freshly installed ones.
# Observed 2026-09-16 on the B200 runner: the baked cutlass-dsl 4.6.0 `nvidia_cutlass_dsl/lib/`
# runtime reappeared in the test session, DSL runtime discovery prefers `lib/` over `cu13/lib/`,
# and every kernel failed to JIT-link with "Symbols not found: CuteDSLRT_TVMFFISetRaisedCudaError"
# although the on-disk upper layer was correct. Load-dependent, so it is intermittent.
#
# Three layers of defence: (1) wait until nothing holds the image before opening the next session,
# (2) verify in a FRESH session that the runtime stack is exactly the installed one (re-provision
# once, then fail loudly), (3) pin the verified runtime via CUTE_DSL_LIBS so JIT linking never
# depends on directory discovery.

def _pids_holding(path: str) -> list[int]:
    """PIDs (visible to this user) with `path` open. Only the fuse2fs of a live session holds it."""
    holders = []
...
# Runs inside a fresh session after provisioning. Prints "CUTE_DSL_LIBS=<paths>" on success.
...
# 1. Exactly one visible distribution per runtime package. Two = the SIF-baked copy leaked through.
seen = {}
for name, d in dists():
...
# 2. FA4 must import from the checked-out repo (editable install), not from a baked copy.
...
# 3. The CuTeDSL runtime the JIT will link must be a file of the INSTALLED cutlass-dsl distribution.
...
def verify_overlay(repo_root: Path, base_env: dict[str, str], sif: str, work_dir: str, overlay: str) -> str | None:
    """Open a fresh session on the provisioned overlay and check the runtime stack is consistent.

    Returns the CUTE_DSL_LIBS value to pin for the test sessions, or None if verification failed.
    """
...
def provision_verified_overlay(
    ...
    """Create + provision + verify the overlay, re-provisioning once on a failed verification."""
    for attempt in (1, 2):
    ...
    raise SystemExit(
        "Overlay verification failed twice: the test session would run against a mixed SIF-baked / "
        "runtime-installed package tree. See the OVERLAY VERIFY FAILED lines above."
    )
...
def run_step(step: Step, repo_root: Path, base_env: dict[str, str], sif: str, work_dir: str, overlay: str, dsl_libs: str | None = None) -> None:
    ...
    if dsl_libs:
        # Pin the runtime verified by verify_overlay(); the JIT link no longer depends on which
        # `lib/` directories happen to be visible in this session.
