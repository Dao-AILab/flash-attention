...
CK_API_VARIANT_DROP_SEED_COMMIT = "c24fae234600aa2863e945d072e6f5b3aec2a6b2"
CK_API_CURRENT_COMMIT = "7c6430eca04e62454217630ae2a0bbd70ff50a00"
AITER_SUBMODULE = "third_party/aiter"
AITER_CK_DIR = os.path.join(AITER_SUBMODULE, "3rdparty", "composable_kernel")
...
def maybe_checkout_ck_git_ref() -> None:
    if not CK_GIT_REF:
        return
    subprocess.run(["git", "-C", AITER_CK_DIR, "checkout", CK_GIT_REF], check=True)


def get_ck_git_commit(ck_dir: str):
    try:
        return subprocess.check_output(
            ["git", "-C", ck_dir, "rev-parse", "HEAD"],
            text=True,
        ).strip()
    except Exception:
        return None
...
def ck_commit_is_at_least(ck_dir: str, current_commit: str, minimum_commit: str) -> bool:
    if current_commit is None:
        return False
    if current_commit == minimum_commit:
        return True
    result = subprocess.run(
        ["git", "-C", ck_dir, "merge-base", "--is-ancestor", minimum_commit, current_commit],
        check=False,
    )
    return result.returncode == 0


def detect_ck_current_api_by_headers(ck_dir: str) -> Optional[bool]:
    fmha_bwd = Path(ck_dir) / "example" / "ck_tile" / "01_fmha" / "fmha_bwd.hpp"
    fmha_fwd = Path(ck_dir) / "example" / "ck_tile" / "01_fmha" / "fmha_fwd.hpp"
    if not fmha_bwd.exists() or not fmha_fwd.exists():
        return None

    bwd_text = fmha_bwd.read_text(encoding="utf-8")
    fwd_text = fmha_fwd.read_text(encoding="utf-8")
    has_variant_drop_seed = (
        "std::variant<std::pair<uint64_t, uint64_t>, std::pair<const void*, const void*>>"
        in bwd_text
        and "std::variant<std::pair<uint64_t, uint64_t>, std::pair<const void*, const void*>>"
        in fwd_text
    )
    has_current_bwd_traits = "struct fmha_bwd_traits" in bwd_text and "int seqlen_q;" in bwd_text
    return has_variant_drop_seed and has_current_bwd_traits


def write_ck_build_config(ck_dir: str) -> None:
    ck_commit = get_ck_git_commit(ck_dir)
    use_current_api = detect_ck_current_api_by_headers(ck_dir)
    if (
        use_current_api is None
        and
        ck_commit is not None
        and ck_commit_is_at_least(ck_dir, ck_commit, CK_API_VARIANT_DROP_SEED_COMMIT)
        and not ck_commit_is_at_least(ck_dir, ck_commit, CK_API_CURRENT_COMMIT)
    ):
        raise RuntimeError(
            "Selected composable_kernel commit is in an unsupported FMHA API transition range. "
            f"Choose a legacy commit before {CK_API_VARIANT_DROP_SEED_COMMIT[:9]} or a current "
            f"commit at/after {CK_API_CURRENT_COMMIT[:9]}. Current commit: {ck_commit}."
        )
    if use_current_api is None:
        use_current_api = (
            ck_commit is not None and ck_commit_is_at_least(ck_dir, ck_commit, CK_API_CURRENT_COMMIT)
        )
    config_path = Path(this_dir) / "csrc" / "flash_attn_ck" / "ck_build_config.hpp"
    content = [
        "// Generated by setup.py. Do not edit manually.",
        "#pragma once",
        "",
        f'#define FLASHATTN_CK_GIT_COMMIT "{ck_commit or "unknown"}"',
        f"#define FLASHATTN_CK_USE_CURRENT_API {1 if use_current_api else 0}",
        "",
    ]
...
def validate_and_update_archs(archs):
    ...
    allowed_archs = ["native", "gfx90a", "gfx950", "gfx942", "gfx1100", "gfx1101", "gfx1102", "gfx1103"]
...
if IS_ROCM:
    if os.path.isdir(".git"):
        subprocess.run(["git", "submodule", "update", "--init", AITER_SUBMODULE], check=True)
        if ROCM_BACKEND == "ck":
            maybe_checkout_ck_git_ref()
    else:
        assert os.path.isdir(AITER_SUBMODULE), (
            f"{AITER_SUBMODULE} is missing, please use source distribution or git clone"
        )

    if ROCM_BACKEND == "triton":
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "--no-build-isolation", AITER_SUBMODULE],
            check=True,
        )
    elif ROCM_BACKEND == "ck":
        assert os.path.exists(os.path.join(AITER_CK_DIR, "example", "ck_tile", "01_fmha", "generate.py")), (
            f"{AITER_CK_DIR} is missing, please initialize {AITER_SUBMODULE}"
        )
else:
...
if not SKIP_CUDA_BUILD and not IS_ROCM:
    ...
    if ROCM_BACKEND == "ck":
        ...
        if not ck_codegen_targets:
            raise RuntimeError(f"Unable to derive CK codegen targets from archs={codegen_archs}")
        ...
        include_dirs = [
            Path(this_dir) / AITER_CK_DIR / "include",
            Path(this_dir) / AITER_CK_DIR / "library" / "include",
            Path(this_dir) / AITER_CK_DIR / "example" / "ck_tile" / "01_fmha",
        ]
