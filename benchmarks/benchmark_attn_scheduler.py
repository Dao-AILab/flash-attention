...
for headdim in [128]:
    ...
    print(f"{use_dynamic_persistent = }")
    for batch_size, seqlen in bs_seqlen_vals:
        ...
        for causal in [True]:
            # print(f"\n### {headdim = }, {causal = }, {seqlen = }, {batch_size = }, {nheads = }, {nheads_kv = }, {pack_gqa = }, {varlen = }, {deterministic = } ###")
            ...
            if flash_attn_func_python is not None:
                if not varlen:
                    m1_py = time_fwd(flash_attn_func_python, q, k if page_size is None else k_paged, v_fa3 if page_size is None else v_paged, causal=causal, window_size=window_size, learnable_sink=sinks, softcap=softcap, pack_gqa=pack_gqa, use_dynamic_persistent=use_dynamic_persistent, repeats=repeats, verbose=verbose, desc='Fav3 python')
                else:
            ...
            if flash_attn_func_python is not None:
                print(f'FA Python fwd: {m1_py.mean * 1e3:.3f}ms, {(nFLOPS / m1_py.mean * 1e-12):.1f} TFLOPS | {headdim = }, {causal = }, {seqlen = }, {batch_size = }, {nheads = }, {nheads_kv = }, {pack_gqa = }')
                if dtype != torch.float8_e4m3fn and headdim == headdim_v and has_backward:
                    print(f'FA Python bwd: {m1b_py.mean * 1e3:.3f}ms, {(2.5 * nFLOPS / m1b_py.mean * 1e-12):.1f} TFLOPS | {headdim = }, {causal = }, {seqlen = }, {batch_size = }, {nheads = }, {nheads_kv = }, {pack_gqa = }')
