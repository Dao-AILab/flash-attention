...
| `fa4-v*` | `.github/workflows/publish-cute.yml` | `flash_attn/cute` package (`flash-attn-4`) | `setuptools-scm` with `fa4-v*` tags |
...
2. Push that tag.
3. `publish-cute.yml` builds from `flash_attn/cute`, creates a GitHub release, and uploads `flash-attn-4` to PyPI.
