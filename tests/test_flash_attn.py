...
from flash_attn.bert_padding import index_first_axis, pad_input, unpad_input, flash_attn_with_kvcache
...
def attention_ref(
    ...
    if causal:
        ...
        causal_mask = construct_causal_mask(
            seqlen_q, seqlen_k, query_padding_mask, key_padding_mask, max_past, q.device
        )
...
def test_flash_attn_varlen_output(
    ...
    if kvpacked:
        out_ref, attn_ref = attention_kvpacked_ref(
            q, kv, query_padding_mask, key_padding_mask, dropout_p, dropout_mask, causal=causal, max_past=max_past
        )
        out_pt, attn_pt = attention_kvpacked_ref(
            q,
            kv,
