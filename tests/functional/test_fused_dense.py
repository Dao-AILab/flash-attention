...
def test_fused_linear_bias(in_features, out_features, has_bias, return_residual, dtype):
    ...
    model = FusedDense(
        in_features, out_features, bias=has_bias, return_residual=return_residual, device=device, dtype=dtype
    )
    with torch.no_grad():
    ...
    if not return_residual:
        ...
        out, x_copy = model(x)
        x_copy = (
            x_copy[..., :out_features] if out_features < in_features else F.pad(x_copy, (0, out_features - in_features))
        )
        x_pt_copy = (
            x_pt[..., :out_features] if out_features < in_features else F.pad(x_pt, (0, out_features - in_features))
        )
...
def test_fused_mlp(
    in_features, out_features, activation, has_bias1, has_bias2, return_residual, checkpoint_lvl, heuristic, dtype
):
    device = "cuda"
    ...
    x_pt = torch.randn(batch_size, seqlen, in_features, device=device, dtype=dtype, requires_grad=True)
    x = x_pt.detach().clone().requires_grad_()
    model_pt_fc1 = torch.nn.Linear(in_features, out_features, bias=has_bias1, device=device, dtype=dtype)
