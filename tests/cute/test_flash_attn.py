...
@pytest.mark.parametrize("seqlen_q,seqlen_k,token_chunk", [(256, 512, None), (1024, 1024, None), (256, 512, 200)])
@maybe_fake_tensor_mode(USE_FAKE_TENSOR)
def test_flash_attn_mla_sparse_token_pairs(seqlen_q, seqlen_k, token_chunk, nheads, shared_kv, causal, dtype):
    ...
    Chunked cases keep pairs intact and exercise a partial final chunk.
    """
    if not IS_SM100:
        pytest.skip()
    device = "cuda"
    torch.random.manual_seed(0)
    batch_size = 2 if token_chunk is None else 1
    nheads_kv, hdim, hdimv = 1, 64, 512
    topk_len = 128
    ...
    with pytest.raises(ValueError, match="requires an even gather_bwd_token_chunk"):
        flash_attn_func(
            q, k, v, qv=qv, gather_kv_indices=pair_indices, causal=causal,
            gather_kv_token_pairs=True, gather_bwd_token_chunk=201,
        )
    out, lse = flash_attn_func(
        q, k, v, qv=qv, gather_kv_indices=pair_indices, causal=causal, pack_gqa=True,
        gather_kv_token_pairs=True, gather_bwd_token_chunk=token_chunk,
    )
