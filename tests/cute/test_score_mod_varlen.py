...
def run_flex_varlen_ref(
    ...
    if cu_seqlens_q is not None:
        ...
        # Neither varlen - shouldn't happen in varlen tests but handled
    ...
    for i in range(batch_size):
        ...
        if cu_seqlens_q is not None:
            ...
            q_slice = q[start_q:end_q].unsqueeze(0).transpose(1, 2) # (1, H, Sq, D)
        else:
            # Not varlen Q - use batch dimension
            start_q, end_q = None, None
            # q is (B, S, H, D), take slice i -> (1, S, H, D)
            q_slice_batched = q[i:i+1] 
            q_slice = q_slice_batched.transpose(1, 2) # (1, H, S, D)
        ...
        if cu_seqlens_k is not None:
            ...
            # k is (B, S, H, D)
            k_slice_batched = k[i:i+1]
            v_slice_batched = v[i:i+1]
            k_slice = k_slice_batched.transpose(1, 2)
            v_slice = v_slice_batched.transpose(1, 2)
        ...
        # Output is (1, H, Sq, D) -> (Sq, H, D)
        results.append(out_slice.transpose(1, 2).squeeze(0))

    # Reassemble result based on input Q layout
    if cu_seqlens_q is None:
        # Batched Q case: Stack to recreate (B, S, H, D)
        return torch.stack(results, dim=0)
    else:
        # Packed Q case: Concat to recreate (Total_Q, H, D)
...
def test_varlen_kv_idx_one_hot_extraction(seqlens_q, seqlens_k, dtype):
    """
    Direct test using one-hot V encoding to extract kv_idx values.
    
    Strategy: For each sequence, we encode the logical kv position directly into V.
    After attention, we can read back which positions were accessed.
    """
    torch.random.manual_seed(42)
    
    num_heads = 1
    head_dim = 8  # Only need 1 dimension to store position
    total_q = sum(seqlens_q)
    total_k = sum(seqlens_k)
    num_seqs = len(seqlens_q)
    ...
    # Encode logical position into V
    # V[global_k] = logical_k (position within its sequence)
    v = torch.zeros(total_k, num_heads, head_dim, device="cuda", dtype=dtype)
    for seq_idx in range(num_seqs):
        start_k = cu_seqlens_k[seq_idx].item()
        end_k = cu_seqlens_k[seq_idx + 1].item()
        seqlen_k = end_k - start_k
        # Store logical kv indices: 0, 1, 2, ..., seqlen_k-1
        v[start_k:end_k, 0, 0] = torch.arange(seqlen_k, dtype=dtype, device="cuda")
    
    # Score mod that focuses on a specific logical kv_idx
    test_cases = []
    for target_kv in [0, 5, 10]:  # Test first, middle positions
        def make_score_mod(target):
            @cute.jit
            def score_mod_focus_on_kv(tSrS_ssa, b_idx, h_idx, q_idx, kv_idx, aux_tensors):
                # Give huge score to target kv_idx, small score to others
                matches = operator.eq(kv_idx, cute.full_like(kv_idx, target))
                return cute.where(matches, 
                                cute.full_like(tSrS_ssa, 50.0),
                                cute.full_like(tSrS_ssa, -50.0))
            return score_mod_focus_on_kv
        
        score_mod = make_score_mod(target_kv)
        ...
        print(f"\nTarget kv_idx={target_kv}:")
        
        # Check each sequence
        for seq_idx in range(num_seqs):
            start_q = cu_seqlens_q[seq_idx].item()
            end_q = cu_seqlens_q[seq_idx + 1].item()
            seqlen_k = seqlens_k[seq_idx]
            
            seq_out = out_cute[start_q:end_q, 0, 0]
            mean_output = seq_out.mean().item()
            
            # If kv_idx is logical:
            #   - If target_kv < seqlen_k: should attend to position target_kv, output ~target_kv
            #   - If target_kv >= seqlen_k: no match, undefined (but shouldn't crash)
            
            # If kv_idx is global in seq 1:
            #   - target_kv=0 would never match (kv_idx starts at 64)
            #   - target_kv=5 would never match
            #   - Output would be very different
            
            if target_kv < seqlen_k:
                expected = float(target_kv)
                print(f"  Seq {seq_idx} (len={seqlen_k}): mean={mean_output:.2f} "
                      f"(expected ~{expected:.2f})")
                
                if seq_idx > 0:  # Critical check for seq 1+
                    # If kv_idx were global, we'd never see target_kv in range
                    # The output would be very different
                    if abs(mean_output - expected) > 2.0:
                        # Check if it looks like global indexing
                        start_k_global = cu_seqlens_k[seq_idx].item()
                        if target_kv < start_k_global:
                            raise AssertionError(
                                f"Seq {seq_idx}: output {mean_output:.2f} doesn't match "
                                f"expected {expected:.2f}. This suggests kv_idx is GLOBAL "
                                f"(would start at {start_k_global}, so target {target_kv} "
                                f"is never seen)."
                            )
                
                assert abs(mean_output - expected) < 2.0, (
                    f"Seq {seq_idx}: output {mean_output:.2f} should be ~{expected:.2f}"
                )
            else:
                print(f"  Seq {seq_idx} (len={seqlen_k}): target_kv={target_kv} "
                      f"out of range, output={mean_output:.2f}")


@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])  
def test_varlen_both_q_and_kv_idx_extraction(dtype):
    """
    Test that extracts BOTH q_idx and kv_idx to verify they're both logical.
    
    Uses a mathematical relationship that only works if both are logical.
    """
    torch.random.manual_seed(42)
    ...
    total = sum(seqlens)
    num_heads = 1
    head_dim = 2  # Need 2 dims: one for q_idx, one for kv_idx
    ...
    # V encodes [logical_q_pos, logical_kv_pos] 
    # We'll use score_mod to embed q_idx, and V already has kv position
    v = torch.zeros(total, num_heads, head_dim, device="cuda", dtype=dtype)
    for seq_idx in range(len(seqlens)):
        start = cu_seqlens[seq_idx].item()
        end = cu_seqlens[seq_idx + 1].item()
        seqlen = end - start
        # dim 0: will be filled by q_idx from score_mod
        # dim 1: logical kv position
        v[start:end, 0, 1] = torch.arange(seqlen, dtype=dtype, device="cuda")
    
    @cute.jit
    def score_mod_embed_q_idx(tSrS_ssa, b_idx, h_idx, q_idx, kv_idx, aux_tensors):
        # Return q_idx * 0.1 to embed it in scores
        # After attention, output will contain weighted combination
        return q_idx.to(cutlass.Float32) * cute.full_like(tSrS_ssa, 0.1)
    
    out_cute = run_cute_flash(
        q, k, v, score_mod_embed_q_idx,
        aux_tensors=None,
        cu_seqlens_q=cu_seqlens,
        cu_seqlens_k=cu_seqlens,
        pack_gqa=False,
    ...
    print(f"\nBoth q and kv idx extraction:")
    
    # For seq 1, check specific query positions
    for q_pos in [0, 10, 50]:
        global_q = 64 + q_pos  # Global position in packed format
        kv_dim = out_cute[global_q, 0, 1].item()
        
        # If both q_idx and kv_idx are logical:
        # - q_idx for this position is q_pos (0, 10, or 50)
        # - Attention focuses on kv positions based on score q_pos * 0.1
        # - Output kv_dim should be a reasonable kv position (0 to 127)
        
        # If kv_idx were global:
        # - kv values would be in range [64, 191]
        # - Output would be much larger
        
        print(f"  Seq 1, q_pos={q_pos}: kv_dim output={kv_dim:.2f}")
        
        # Sanity check: should be in reasonable range for logical indexing
        assert 0 <= kv_dim <= 128, (
            f"kv_dim={kv_dim:.2f} is out of logical range [0, 128]. "
            f"If this is closer to [64, 191], kv_idx may be global."
        )
        
        if kv_dim > 140:  # Clearly in the global range for seq 1
            raise AssertionError(
                f"kv_dim={kv_dim:.2f} suggests GLOBAL kv_idx! "
                f"Expected logical range [0, 128], got value suggesting [64, 191]."
            )
    
    print(f"  ✓ Both q_idx and kv_idx appear to be logical")


@pytest.mark.parametrize("dtype", [torch.float16])
@pytest.mark.parametrize("varlen_mode", ["both", "q_only", "k_only"])
def test_varlen_index_arithmetic_check(varlen_mode, dtype):
    """
    Arithmetic verification: compute (q_idx - kv_idx) and check bounds.
    
    This should work regardless of attention mechanics.
    """
    torch.random.manual_seed(42)
    ...
    if varlen_mode == "both":
        cu_seqlens_q = torch.tensor([0, 64, 192], device="cuda", dtype=torch.int32)
        cu_seqlens_k = torch.tensor([0, 64, 192], device="cuda", dtype=torch.int32)
        q = torch.randn(192, num_heads, head_dim, device="cuda", dtype=dtype)
        k = torch.randn(192, num_heads, head_dim, device="cuda", dtype=dtype)
        v = torch.ones(192, num_heads, head_dim, device="cuda", dtype=dtype)
    elif varlen_mode == "q_only":
        cu_seqlens_q = torch.tensor([0, 64, 192], device="cuda", dtype=torch.int32)
        cu_seqlens_k = None
        q = torch.randn(192, num_heads, head_dim, device="cuda", dtype=dtype)
        k = torch.randn(2, 128, num_heads, head_dim, device="cuda", dtype=dtype)  # Batched
        v = torch.ones(2, 128, num_heads, head_dim, device="cuda", dtype=dtype)
    else:  # k_only
        cu_seqlens_q = None
        cu_seqlens_k = torch.tensor([0, 64, 192], device="cuda", dtype=torch.int32)
        q = torch.randn(2, 64, num_heads, head_dim, device="cuda", dtype=dtype)  # Batched
        k = torch.randn(192, num_heads, head_dim, device="cuda", dtype=dtype)
        v = torch.ones(192, num_heads, head_dim, device="cuda", dtype=dtype)
    
    @cute.jit
    def score_mod_relative_diff(tSrS_ssa, b_idx, h_idx, q_idx, kv_idx, aux_tensors):
        # Compute q_idx - kv_idx and return scaled version
        # For logical indexing, this should be in range [-(seqlen-1), (seqlen-1)]
        # For global indexing in seq 1, this would be in range [64-191, 128-64] = [-127, 64]
        diff = q_idx - kv_idx
        return diff.to(cutlass.Float32) * cute.full_like(tSrS_ssa, 0.01)
    
    out_cute = run_cute_flash(
        q, k, v, score_mod_relative_diff,
        aux_tensors=None,
        cu_seqlens_q=cu_seqlens_q,
        cu_seqlens_k=cu_seqlens_k,
        pack_gqa=False,
    ...
    print(f"\nArithmetic check ({varlen_mode}):")
    
    # Extract the output range for seq 1 (if varlen Q)
    if varlen_mode in ["both", "q_only"]:
        seq1_out = out_cute[64:192]
        min_val = seq1_out.min().item()
        max_val = seq1_out.max().item()
        
        print(f"  Seq 1 output range: [{min_val:.4f}, {max_val:.4f}]")
        print(f"    Logical indexing: expect ~[-1.27, 1.27]")
        print(f"    Global indexing:  would be ~[-1.27, 0.64] (shifted)")
        
        # Key check: maximum value
        # For logical: max(q_idx - kv_idx) when both are same sequence ≈ seqlen
        # For global in seq 1: max(q_idx - kv_idx) = max(0..127 - 64..191) = 0
        
        if max_val < 0.1:
            raise AssertionError(
                f"Max value {max_val:.4f} is too small! "
                f"For logical indexing, should see positive values up to ~1.27. "
                f"This suggests kv_idx is GLOBAL (always > q_idx)."
            )
        
        assert max_val > 0.5, f"Max value {max_val:.4f} too small for logical indexing"
        assert max_val < 1.5, f"Max value {max_val:.4f} too large"
    
    print(f"  ✓ Index arithmetic consistent with logical indexing")


@pytest.mark.parametrize("dtype", [torch.float16])
def test_offset_diagnostic(dtype):
    """Diagnostic: Check if offset_k is being set."""
    
    @cute.jit
    def score_mod_check_offset(tSrS_ssa, b_idx, h_idx, q_idx, kv_idx, aux_tensors):
        # If offset is applied: seq 0 sees kv_idx in [0,64), seq 1 sees [0,128)
        # If offset NOT applied: seq 0 sees [0,64), seq 1 sees [64,192)
        # We can detect this by checking if kv_idx ever exceeds 64 in any sequence
        return tSrS_ssa + kv_idx.to(cutlass.Float32) * cute.full_like(tSrS_ssa, 0.0)  # No-op but forces kv_idx evaluation
    
    seqlens = [64, 128]
    cu_seqlens = torch.tensor([0, 64, 192], device="cuda", dtype=torch.int32)
    
    q = torch.randn(192, 1, 8, device="cuda", dtype=dtype)
    k = torch.randn(192, 1, 8, device="cuda", dtype=dtype)
    v = torch.ones(192, 1, 8, device="cuda", dtype=dtype)
    
    # Try to trigger the kernel
    out_cute = run_cute_flash(
        q, k, v, score_mod_check_offset,
        aux_tensors=None,  # Critical: no aux tensors
        cu_seqlens_q=cu_seqlens,
        cu_seqlens_k=cu_seqlens,
        pack_gqa=False,
    ...
    print(f"If this runs without error, check kernel logs for kv_idx values")
