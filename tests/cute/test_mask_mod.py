...
from mask_mod_definitions import (
    ...
    get_cute_sliding_window_mask,
    get_flex_sliding_window_mask,
)
...
def _run_mask_test(
    ...
    if needs_backward:
        ...
        _assert_bwd_matches_flex_reference(
            q,
            k,
            v,
            dq_cute,
...
def _assert_bwd_matches_flex_reference(
    q,
    k,
    v,
    dq,
    ...
    (
        ...
        dk_bound,
        dv_bound,
    ) = _compute_bwd_reference_errors(
        q, k, v, dq, dk, dv, grad_out, mask_mod_flex, block_size, dtype
    )
    assert cute_dq_err <= dq_bound, f"dQ error too large: {cute_dq_err:.2e}"
    assert cute_dk_err <= dk_bound, f"dK error too large: {cute_dk_err:.2e}"
    assert cute_dv_err <= dv_bound, f"dV error too large: {cute_dv_err:.2e}"


def _compute_bwd_reference_errors(
    q,
    k,
    v,
    dq,
    ...
    _, dq_pt, dk_pt, dv_pt = run_flex_reference_bwd(q, k, v, block_mask, grad_out)
    ...
    cute_dq_err = (dq - dq_ref).abs().max().item()
    cute_dk_err = (dk - dk_ref).abs().max().item()
    cute_dv_err = (dv - dv_ref).abs().max().item()
    return (
        cute_dq_err,
        cute_dk_err,
        cute_dv_err,
        bwd_rtol * pt_dq_err + dq_atol,
        bwd_rtol * pt_dk_err + dk_atol,
        bwd_rtol * pt_dv_err + dv_atol,
    )
...
def test_gqa_expand_stride_zero_bug():
    ...
    out_ref = compute_reference_flex_attn(tensors_for_ref, mask_mod_flex)
    out_ref_fp32 = compute_reference_flex_attn(tensors_fp32, mask_mod_flex)
...
def _run_deterministic_bwd_repeats(
    q,
    k,
    v,
    out_cute,
...
def test_block_sparse_bwd_deterministic(seqlen_q, seqlen_k, mask_name, window_size, spt, kv_mode):
    ...
    if kv_mode != "mha" and (
        spt or mask_name != "block_diagonal" or (seqlen_q, seqlen_k) != (256, 256)
    ):
        pytest.skip("gqa/mqa deterministic sanity checks run on the block_diagonal 256x256 non-spt anchor")
    ...
    reference_mask_mod_flex = mask_mod_flex
    ...
    if spt and mask_name == "sliding_window":
        offset = seqlen_k - seqlen_q
        assert window_size is not None
        mask_mod_cute = get_cute_sliding_window_mask(window_size, 0, offset)
        reference_mask_mod_flex = get_flex_sliding_window_mask(window_size, 0, offset)
    ...
    block_sparse_mask_fwd, block_sparse_mask_bwd = _build_block_sparse_masks_for_bwd(
        mask_mod_flex=reference_mask_mod_flex,
    ...
    grad_out = torch.randn_like(out_cute)
    dq0, dk0, dv0 = _run_deterministic_bwd_repeats(
        q,
        k,
        v,
        out_cute,
        ...
        tile_m,
        tile_n,
        num_repeats=3 if spt else (10 if kv_mode != "mha" else 50),
        aux_tensors=aux_tensors_arg,
    )
    _assert_bwd_matches_flex_reference(
        q,
        ...
        dv0,
        grad_out,
        reference_mask_mod_flex,
        block_sparse_mask_fwd.block_size,
        dtype,
    )
