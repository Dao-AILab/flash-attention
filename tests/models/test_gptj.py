...
def test_gptj_state_dict(model_name):
    ...
    rotary_inv_freq_keys = {f"transformer.layers.{l}.mixer.rotary_emb.inv_freq" for l in range(config.n_layer)}
...
def test_gptj_optimized(model_name):
    ...
    input_ids = torch.randint(0, config.vocab_size, (batch_size, max_seqlen), dtype=torch.long, device=device)
    with torch.no_grad():
    ...
    model_hf = GPTJForCausalLM.from_pretrained(model_name, torch_dtype=dtype, device_map={"": device})
    model_hf.eval()
...
def test_gptj_generation(model_name):
    ...
    seqlen = 100
    max_length = 150
    input_ids = torch.randint(0, config.vocab_size, (batch_size, seqlen), dtype=torch.long, device=device)
