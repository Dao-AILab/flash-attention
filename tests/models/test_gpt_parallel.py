...
def test_gpt_parallel(dim, has_pos_emb, sequence_parallel, world_size, dtype):
    ...
    assert torch.allclose(
        out, out_pt[:, rank * partition_vocab_size : (rank + 1) * partition_vocab_size], rtol=rtol, atol=atol
    )
    ...
    grad_dict = shard_state_dict_tp({k: v.grad for k, v in model_pt.named_parameters()}, config, world_size, rank)
    ...
    assert torch.allclose(
        model.transformer.ln_f.weight.grad, grad_dict["transformer.ln_f.weight"], rtol=rtol, atol=atol
    )
    assert torch.allclose(model.transformer.ln_f.bias.grad, grad_dict["transformer.ln_f.bias"], rtol=rtol, atol=atol)
