...
def generate_qkv(q, k, v, query_padding_mask=None, key_padding_mask=None,
    ...
    if key_padding_mask is not None:
        k_unpad, indices_k, cu_seqlens_k, max_seqlen_kv = unpad_input(k, key_padding_mask)
        v_unpad, _, _, _ = unpad_input(v, key_padding_mask)
    else:
        k_unpad = rearrange(k, 'b s h d -> (b s) h d')
        v_unpad = rearrange(v, 'b s h d -> (b s) h d')
        cu_seqlens_k = torch.arange(0, (batch_size + 1) * seqlen_kv, step=seqlen_kv, dtype=torch.int32,
    ...
    if qkvpacked:
        ...
        if key_padding_mask is not None:
            dkv_pad_fn = lambda dkv_unpad: pad_input(dkv_unpad, indices_k, batch_size, seqlen_kv)
        else:
        ...
        return (q_unpad.detach().requires_grad_(), kv_unpad.detach().requires_grad_(),
                cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_kv,
                q.detach().requires_grad_(), kv.detach().requires_grad_(),
                output_pad_fn, dq_pad_fn, dkv_pad_fn)
    else:
        dq_pad_fn = output_pad_fn
        if key_padding_mask is not None:
            dk_pad_fn = lambda dk_unpad: pad_input(dk_unpad, indices_k, batch_size, seqlen_kv)
        else:
...
def attention_ref(q, k, v, query_padding_mask=None, key_padding_mask=None, dropout_p=0.0,
    ...
    if causal:
        causal_mask = torch.triu(torch.ones(seqlen_q, seqlen_kv, dtype=torch.bool, device=q.device), 1)
...
def convert_flash_attn_Z_to_dropout(Z, query_padding_mask, key_padding_mask, head_dim, dropout_p):
    ...
    seqlen_q_unpadded = query_padding_mask.shape[-1] if query_padding_mask is not None else seqlen_q
    seqlen_kv_unpadded = key_padding_mask.shape[-1] if key_padding_mask is not None else seqlen_kv
  
    if seqlen_q_unpadded > seqlen_q:
      Z_converted = F.pad(Z_converted, (0, 0, 0, seqlen_q_unpadded - seqlen_q))
      Z_converted = Z_converted.masked_fill(rearrange(~query_padding_mask, 'b s -> b 1 s 1'), 0.0)
  
    if seqlen_kv_unpadded > seqlen_kv:
      Z_converted = F.pad(Z_converted, (0, seqlen_kv_unpadded - seqlen_kv))
      Z_converted = Z_converted.masked_fill(rearrange(~key_padding_mask, 'b s -> b 1 1 s'), 0.0)
        
    return torch.floor(255. - 255.*dropout_p - Z_converted)
...
def get_dropout_fraction(dropout_mask, query_padding_mask=None, key_padding_mask=None, causal=False):
    ...
    if causal:
        causal_mask = torch.triu(torch.ones(seqlen_q, seqlen_kv, dtype=torch.bool,
...
def test_flash_attn_qkvpacked(seqlen, d, dropout_p, causal, dtype):
    ...
    if dropout_p > 0.0:
        S_dmask_converted = convert_flash_attn_Z_to_dropout(
            S_dmask, None, None, d, dropout_p)[:, :, :seqlen, :seqlen]
...
def test_flash_attn_varlen_qkvpacked(seqlen, d, dropout_p, causal, dtype):
    ...
    if dropout_p > 0.0:
        S_dmask_converted = convert_flash_attn_Z_to_dropout(
            S_dmask, key_padding_mask, key_padding_mask, d, dropout_p)[:, :, :seqlen, :seqlen]
        dropout_mask = S_dmask_converted >= 0
        # attn_unnorm = S_dmask_converted.abs()
        # attn = normalize_flash_attn_S(attn_unnorm, qkv[:, :, 0], qkv[:, :, 1], qkv[:, :, 2],
...
# @pytest.mark.parametrize('d', [64])
@pytest.mark.parametrize('seqlen_q,seqlen_kv', [(113, 203), (128, 217), (113, 211), (108, 256), (256, 512), (512, 256), (1024, 1024), (1023, 1024), (1024, 1023), (2048, 2048)])
# @pytest.mark.parametrize('seqlen_q,seqlen_kv', [(128, 128)])
@pytest.mark.parametrize('dropout_p', [0.0, 0.17])
# @pytest.mark.parametrize('dropout_p', [0.17])
def test_flash_attn_output(seqlen_q, seqlen_kv, d, dropout_p, causal, mha_type, dtype, kvpacked):
    if max(seqlen_q, seqlen_kv) >= 2048 and torch.cuda.get_device_properties('cuda').total_memory <= 16 * 2**30:
        pytest.skip()  # Reference implementation OOM
    device = 'cuda'
    # set seed
    torch.random.manual_seed(0)
    ...
    if kvpacked:
        kv = torch.randn(batch_size, seqlen_kv, 2, nheads_k, d, device=device, dtype=dtype,
                         requires_grad=True)
    else:
        k = torch.randn(batch_size, seqlen_kv, nheads_k, d, device=device, dtype=dtype,
                        requires_grad=True)
        v = torch.randn(batch_size, seqlen_kv, nheads_k, d, device=device, dtype=dtype,
                        requires_grad=True)
    ...
    if dropout_p > 0.0:
        S_dmask_converted = convert_flash_attn_Z_to_dropout(
            S_dmask, None, None, d, dropout_p)[:, :, :seqlen_q, :seqlen_kv]
        dropout_mask = S_dmask_converted >= 0
        # attn_unnorm = S_dmask_converted.abs()
        if kvpacked:
...
@pytest.mark.parametrize('dropout_p', [0.0, 0.17])
# @pytest.mark.parametrize('dropout_p', [0.17])
def test_flash_attn_varlen_output(seqlen_q, seqlen_kv, d, dropout_p, causal, mha_type, dtype,
                                  kvpacked):
    if max(seqlen_q, seqlen_kv) >= 2048 and torch.cuda.get_device_properties('cuda').total_memory <= 16 * 2**30:
        pytest.skip()  # Reference implementation OOM
    device = 'cuda'
    ...
    if kvpacked:
        (q_unpad, kv_unpad, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_kv, q, kv,
        ...
        out_unpad, sm_lse, S_dmask = flash_attn_varlen_kvpacked_func(
            q_unpad, kv_unpad, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_kv,
            dropout_p, return_attn_probs=True, causal=causal
        )
    else:
        (q_unpad, k_unpad, v_unpad, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_kv, q, k, v,
        ...
        out_unpad, sm_lse, S_dmask = flash_attn_varlen_func(
            q_unpad, k_unpad, v_unpad, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_kv,
            dropout_p, return_attn_probs=True, causal=causal
        )
    out = output_pad_fn(out_unpad)
    if dropout_p > 0.0:
        S_dmask_converted = convert_flash_attn_Z_to_dropout(
            S_dmask, query_padding_mask, key_padding_mask, d, dropout_p)[:, :, :seqlen_q, :seqlen_kv]
        dropout_mask = S_dmask_converted >= 0
        # attn_unnorm = S_dmask_converted.abs()
        if kvpacked:
            kv_rep = repeat(kv, "b s two h d -> b s two (h g) d", g=nheads // nheads_k)
