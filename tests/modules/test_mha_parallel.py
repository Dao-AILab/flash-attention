...
def test_mha_parallel(embed_dim, head_dim, sequence_parallel, world_size, dtype):
    ...
    x_pt = torch.randn(batch_size * seqlen, embed_dim, device=device, dtype=dtype, requires_grad=True)
    ...
    model_pt = MHA(
        embed_dim, num_heads, rotary_emb_dim=int(head_dim // 2), use_flash_attn=True, device=device, dtype=dtype
    )
    ...
    with torch.no_grad():
        ...
        model.out_proj.weight.copy_(model_pt.out_proj.weight[:, rank * partition_dim : (rank + 1) * partition_dim])
        if rank == 0:
