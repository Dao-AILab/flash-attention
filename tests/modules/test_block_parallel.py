...
def test_block_parallel(dim, sequence_parallel, world_size, dtype):
    ...
    mixer_cls_pt = partial(
        MHA, num_heads=num_heads, rotary_emb_dim=int(head_dim // 2), use_flash_attn=True, device=device, dtype=dtype
    )
    ...
    out_pt, out_residual_pt = model_pt(
        rearrange(x_pt, "(b s) d -> b s d", s=seqlen), rearrange(residual_pt, "(b s) d -> b s d", s=seqlen)
    )
    ...
    if rank == 0:
        assert torch.allclose(
            model.mixer.out_proj.bias.grad, model_pt.mixer.out_proj.bias.grad, rtol=rtol, atol=atol * 5
        )
    assert torch.allclose(
    ...
    if rank == 0:
        assert torch.allclose(model.mlp.fc2.bias.grad, model_pt.mlp.fc2.bias.grad, rtol=rtol, atol=atol * 5)
