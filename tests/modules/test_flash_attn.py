...
def generate_qkv(x, Wqkv, nheads, query_padding_mask=None, key_padding_mask=None, kvpacked=False, qkvpacked=False):
    """
    Arguments:
    ...
    if query_padding_mask is not None:
        ...
        cu_seqlens_q = torch.arange(0, (batch_size + 1) * seqlen, step=seqlen, dtype=torch.int32, device=q_unpad.device)
        max_seqlen_q = seqlen
    ...
    if key_padding_mask is not None:
        ...
        cu_seqlens_k = torch.arange(0, (batch_size + 1) * seqlen, step=seqlen, dtype=torch.int32, device=q_unpad.device)
        max_seqlen_k = seqlen
    ...
    if qkvpacked:
        ...
        q, k, v = [rearrange(z, "b s (h d) -> b s h d", h=nheads).detach().requires_grad_() for z in [q, k, v]]
        dq_pad_fn = output_pad_fn
        if key_padding_mask is not None:
...
def convert_flash_attn_S_to_softmax(S, query_padding_mask, key_padding_mask, head_dim, is_dropout, causal=False):
    ...
    S_converted = rearrange(
        S_flat,
        "b h (loop nsteps mmas_n warps_n eight t r c0 c1) -> b h (nsteps r eight) (loop mmas_n warps_n c0 t c1)",
        loop=loop_steps,
        nsteps=seqlen_q // 16,
        mmas_n=mmas_n,
        warps_n=warps_n,
...
def normalize_flash_attn_S(
    attn_unnorm, q, k, v, query_padding_mask=None, key_padding_mask=None, is_dropout=False, causal=False
):
    """
    Arguments:
        q: (batch_size, seqlen_q, nheads, head_dim)
    ...
    scores_max_block = [torch.amax(scores_block[0], dim=-1)] + [
        torch.maximum(torch.amax(s, dim=-1), lcse) for s, lcse in zip(scores_block[1:], lcse_block[:-1])
    ]
    attn_unnorm_block = attn_unnorm.split(block_size, dim=-1)
    attn_norm = torch.cat(
        [
            a / rearrange(torch.exp(lcse_block[-1] - m), "b h s -> b h s 1")
            for a, m in zip(attn_unnorm_block, scores_max_block)
        ],
        dim=-1,
    )
    if query_padding_mask is not None:
...
def get_dropout_fraction(dropout_mask, query_padding_mask=None, key_padding_mask=None, causal=False):
    ...
    if causal:
        causal_mask = torch.triu(torch.ones(seqlen_q, seqlen_k, dtype=torch.bool, device=dropout_mask.device), 1)
...
def test_flash_attn_unpadded_qkvpacked(seqlen, d, dropout_p, causal, dtype):
    ...
    dropout_fraction = get_dropout_fraction(dropout_mask, key_padding_mask, key_padding_mask, causal=causal).item()

    output_ref, attn_ref = attention_qkvpacked_ref(qkv, key_padding_mask, dropout_p, dropout_mask, causal=causal)
    output_pt, attn_pt = attention_qkvpacked_ref(
        qkv, key_padding_mask, dropout_p, dropout_mask, causal=causal, upcast=False, reorder_ops=True
    )
...
def test_flash_attn_unpadded_kvpacked(seqlen, d, dropout_p, causal, dtype):
    ...
    attn_unnorm = S_dmask_converted.abs()
    attn = normalize_flash_attn_S(
        attn_unnorm, q, kv[:, :, 0], kv[:, :, 1], query_padding_mask, key_padding_mask, dropout_p > 0.0, causal=causal
    )
    dropout_fraction = get_dropout_fraction(dropout_mask, query_padding_mask, key_padding_mask, causal=causal)

    output_ref, attn_ref = attention_kvpacked_ref(
        q, kv, query_padding_mask, key_padding_mask, dropout_p, dropout_mask, causal=causal
    )
    output_pt, attn_pt = attention_kvpacked_ref(
        q,
        kv,
        query_padding_mask,
        key_padding_mask,
    ...
    if is_sm80 or d <= 64:  # Only run backward for d=128 on A100
        ...
        (
            dq_unpad,
            dkv_unpad,
        ) = torch.autograd.grad(output, (q_unpad, kv_unpad), g)
        dq = dq_pad_fn(dq_unpad)
        dkv = dkv_pad_fn(dkv_unpad)
        (
            dq_ref,
            dkv_ref,
        ) = torch.autograd.grad(output_ref, (q, kv), g)
...
def test_flash_attn_unpadded(seqlen, d, dropout_p, causal, dtype):
    ...
    attn_unnorm = S_dmask_converted.abs()
    attn = normalize_flash_attn_S(
        attn_unnorm, q, k, v, query_padding_mask, key_padding_mask, dropout_p > 0.0, causal=causal
    )
    dropout_fraction = get_dropout_fraction(dropout_mask, query_padding_mask, key_padding_mask, causal=causal)
    ...
    if is_sm80 or d <= 64:  # Only run backward for d=128 on A100
        ...
        (
            ...
            dk_unpad,
            dv_unpad,
        ) = torch.autograd.grad(output, (q_unpad, k_unpad, v_unpad), g)
        dq = dq_pad_fn(dq_unpad)
...
def test_flash_attn_split(seqlen, d, dropout_p, causal, dtype):
    ...
    S_dmask1_converted = convert_flash_attn_S_to_softmax(
        S_dmask1,
        key_padding_mask[batch_size0:, :max_seqlen1],
        key_padding_mask[batch_size0:, :max_seqlen1],
        d,
        dropout_p > 0.0,
        causal=causal,
    ...
    padding = (
        S_dmask0_converted.shape[-1] - S_dmask1_converted.shape[-1],
        S_dmask0_converted.shape[-2] - S_dmask1_converted.shape[-2],
    )
    S_dmask_converted = torch.cat(
        [S_dmask0_converted, F.pad(S_dmask1_converted, (0, padding[0], 0, padding[1]))], dim=0
    )
    dropout_mask = S_dmask_converted >= 0
    attn_unnorm = S_dmask_converted.abs()
    attn = normalize_flash_attn_S(
...
def test_flash_attn_race_condition(seqlen, d, dropout_p, causal, dtype):
    ...
    if is_sm80 or d <= 64:  # Only run backward for d=128 on A100
        ...
        (
            ...
            dk_unpad_0,
            dv_unpad_0,
        ) = torch.autograd.grad(output_unpad_0, (q_unpad, k_unpad, v_unpad), g)
    ...
    for _ in range(10):
        ...
        if is_sm80 or d <= 64:  # Only run backward for d=128 on A100
            (
                ...
                dk_unpad,
                dv_unpad,
            ) = torch.autograd.grad(output_unpad, (q_unpad, k_unpad, v_unpad), g)
