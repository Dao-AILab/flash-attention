...
def test_cross_entropy_loss_parallel(vocab_size, world_size, smoothing, inplace_backward, dtype):
    ...
    batch_size = 8
    seqlen = 128
    x_pt = (torch.randn(batch_size * seqlen, vocab_size, device=device, dtype=dtype) * 10).requires_grad_()
    ...
    assert torch.allclose(
        x.grad, x_pt.grad[:, (rank * partition_vocab_size) : (rank + 1) * partition_vocab_size], rtol=rtol, atol=atol
    )
