...
def flash_attn_varlen_qkvpacked_func(
    ...
    Arguments:
        qkv: (total, 3, nheads, headdim) or (total, nheads_packed, headdim), where total = total number of tokens in the batch.
        ...
        deterministic: bool. Whether to use the deterministic implementation of the backward pass,
            which is slightly slower and uses more memory. The forward pass is always deterministic.
        num_heads_q: int. Number of query heads for GQA. Only used when qkv is 3D.
        return_attn_probs: bool. Whether to return the attention probabilities. This option is for
           testing only. The returned probabilities are not guaranteed to be correct
           (they might not have the right scaling).
    Return:
    ...
    return FlashAttnVarlenQKVPackedFunc.apply(
        qkv,
        cu_seqlens,
        cu_seqlens,  # cu_seqlens_k = cu_seqlens_q
        None,  # seqused_q
        None,  # seqused_k
        max_seqlen,
        max_seqlen,  # max_seqlen_k = max_seqlen_q
        softmax_scale,
        causal,
        q_descale, k_descale, v_descale,
        window_size,
