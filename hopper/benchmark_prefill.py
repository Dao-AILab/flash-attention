...
for headdim in [128]:
    for head_swizzle in [True, False]:
        print("\nHead Swizzle is ", head_swizzle)
        for prefill_batches in [16, 8, 4, 2, 1]:
        # for prefill_batches in [4, 4, 4]:
                batch_size = prefill_batches
                # prefill_seqlen_k = (8192 * 8) // prefill_batches
                prefill_seqlen_k = (8192 * 4) // prefill_batches
                seqlen_k = prefill_seqlen_k
                prefill_seqlen_q = prefill_seqlen_k
                seqlen_q = prefill_seqlen_q * prefill_batches
                max_seqlen_q = prefill_seqlen_q
                ...
                cu_seqlens_q = torch.arange(prefill_batches + 1, device=device, dtype=torch.int32) * prefill_seqlen_q
                cache_seqlens = torch.ones(prefill_batches, dtype=torch.int32, device=device) * prefill_seqlen_k 
                ...
                for causal in [True]:
                    print(f"\n### {headdim = }, {nheads = }, {nheads_kv = }, {causal = }, {prefill_seqlen_k = }, {prefill_batches = }, {num_splits = }, {head_swizzle = } ###")
                    ...
                    nFLOPS = prefill_nFLOPS

                    bytes_kv = (prefill_seqlen_k * prefill_batches) * (nheads_kv * headdim * 4)
                    bytes_qo = (prefill_seqlen_q * prefill_batches) * (nheads * headdim * 4) # don't count split partials
