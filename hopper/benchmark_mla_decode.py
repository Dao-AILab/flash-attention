...
num_splits = 0
q = torch.randn(batch_size, seqlen_q, nheads, headdim, dtype=dtype, device=device) * 3
v_cache = torch.randn(batch_size, seqlen, nheads_kv, headdim_v, dtype=dtype, device=device)
k_cache = torch.randn(batch_size, seqlen, nheads_kv, headdim, dtype=dtype, device=device) * 3
...
ideal_h100_time = mem_io / 3.35e12 * 1e6
print(f"Time: {t0 * 1e3:.0f} us, {mem_io * 1e-9 / (t0 * 1e-3):.0f} GB/s")
print(f"Time w CUDA Graph: {t1 * 1e3:.0f} us, {mem_io * 1e-9 / (t1 * 1e-3):.0f} GB/s")
