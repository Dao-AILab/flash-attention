...
#include <torch/csrc/stable/Exception.h>
#include <torch/csrc/stable/library.h>
#include <torch/csrc/stable/tensor.h>
#include <torch/csrc/stable/nn/functional/padding.h>
#include <torch/csrc/stable/cuda/CUDAGuard.h>
#include <torch/csrc/stable/cuda/CUDAStream.h>
#include <torch/csrc/stable/TensorIndexing.h>
#include <torch/standalone/cuda/CUDAContextLight.h>
#include <torch/standalone/nn/options/padding.h>
#include <torch/standalone/util/ArrayRef.h>
#include <torch/standalone/ScalarType.h>
...
#define CHECK_DEVICE(x) STABLE_TORCH_CHECK(x.is_cuda(), #x " must be on CUDA")
#define CHECK_SHAPE(x, ...) STABLE_TORCH_CHECK(x.sizes() == torch::standalone::IntArrayRef({__VA_ARGS__}), #x " must have shape (" #__VA_ARGS__ ")")
#define CHECK_CONTIGUOUS(x) STABLE_TORCH_CHECK(x.is_contiguous(), #x " must be contiguous")

using torch::stable::Tensor;
using torch::stable::cuda::CUDAGuard;
using torch::stable::cuda::getCurrentDeviceProperties;
using torch::standalone::ScalarType;
...
void set_params_fprop(Flash_fwd_params &params,
    ...
    params.is_bf16 = q.dtype() == torch::standalone::kBFloat16;
    params.is_e4m3 = q.dtype() == torch::standalone::kFloat8_e4m3fn;
    ...
    STABLE_TORCH_CHECK(p_dropout < 1.f);
    #ifdef FLASHATTENTION_DISABLE_DROPOUT
        STABLE_TORCH_CHECK(p_dropout == 0.0f, "This flash attention build does not support dropout.");
    #endif
    ...
    params.num_sm = getCurrentDeviceProperties()->multiProcessorCount - sm_margin;

    #ifdef FLASHATTENTION_DISABLE_LOCAL
        STABLE_TORCH_CHECK(!params.is_local, "This flash attention build does not support local attention.");
    #endif
}
...
void run_mha_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    // });
    STABLE_TORCH_CHECK(params.num_splits >= 1);
                    ...
                    SOFTCAP_SWITCH(params.softcap > 0.0, Has_softcap, [&] {
                        if (!params.is_e4m3) {
                            if (params.is_bf16) {
                                ...
                                #endif
                                #else
                                STABLE_TORCH_CHECK(false, "This flash attention build does not support FP16.");
                                #endif
                            }
                        } else {
                            ...
                            #endif
                            #else
                            STABLE_TORCH_CHECK(false, "This flash attention build does not support FP8.");
                            #endif
                        }
                    });
                });
...
void run_mha_fwd_combine(Flash_fwd_params &params, cudaStream_t stream, bool enable_pdl=false) {
    ...
    #else
    STABLE_TORCH_CHECK(false, "This flash attention build does not support combine kernels.");
    #endif
}
...
mha_fwd_get_scheduler_metadata(
    ...
    STABLE_TORCH_CHECK(qkv_dtype == ScalarType::Half || qkv_dtype == ScalarType::BFloat16 || qkv_dtype == ScalarType::Float8_e4m3fn,
                "FlashAttention only supports fp16, bf16, and fp8_e4m3 data type");
    STABLE_TORCH_CHECK(num_heads % num_heads_k == 0, "Number of heads in key/value must divide number of heads in query");
    ...
    CUDAGuard device_guard{(char)seqused_k.get_device()};
    ...
    if (scheduler_needs_semaphore || use_dynamic_split) {
        tile_count_semaphore = torch::stable::empty({int(scheduler_needs_semaphore) + int(use_dynamic_split) * params.b}, opts.dtype(torch::standalone::kInt32));
    ...
    if (params.num_splits_dynamic_ptr) {
        ...
        int const kBlockM = params.arch >= 90 ? std::get<0>(kBlockMN_kernel_args_sm90) : std::get<0>(kBlockMN_kernel_args_sm8x);
        int const kBlockN = params.arch >= 90 ? std::get<1>(kBlockMN_kernel_args_sm90) : std::get<1>(kBlockMN_kernel_args_sm8x);
        auto stream = torch::stable::cuda::getCurrentCUDAStream().stream();
...
mha_fwd(Tensor q,   // (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
    ...
    STABLE_TORCH_CHECK(is_sm8x, "FlashAttention only supports Ampere GPUs or newer.");

    auto q_type = q.scalar_type();
    STABLE_TORCH_CHECK(q_type == ScalarType::Half || q_type == ScalarType::BFloat16 || q_type == ScalarType::Float8_e4m3fn,
                "FlashAttention only supports fp16, bf16, and fp8_e4m3 data type");
    if (dprops->major < 9) {
        STABLE_TORCH_CHECK(q_type == ScalarType::Half || q_type == ScalarType::BFloat16,
                    "FlashAttention on Ampere/Ada cards only supports fp16 and bf16 data type");
    }
    STABLE_TORCH_CHECK(k.scalar_type() == q_type, "query and key must have the same dtype");
    STABLE_TORCH_CHECK(v.scalar_type() == q_type, "query and value must have the same dtype");

    CHECK_DEVICE(q); CHECK_DEVICE(k); CHECK_DEVICE(v);

    STABLE_TORCH_CHECK(q.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    STABLE_TORCH_CHECK(k.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    STABLE_TORCH_CHECK(v.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    ...
    if (paged_KV) {
        ...
        STABLE_TORCH_CHECK(page_table.dtype() == torch::standalone::kInt32, "page_table must have dtype torch.int32");
        STABLE_TORCH_CHECK(page_table.stride(-1) == 1, "page_table must have contiguous last dimension");
    }
    ...
    if (is_varlen_q) {
        ...
        STABLE_TORCH_CHECK(cu_seqlens_q.dtype() == torch::standalone::kInt32, "cu_seqlens_q must have dtype torch.int32");
        STABLE_TORCH_CHECK(max_seqlen_q_.has_value(), "max_seqlen_q must be provided if cu_seqlens_q is provided");
    }
    Tensor cu_seqlens_k;
    bool const is_varlen_k = cu_seqlens_k_.has_value();
    if (is_varlen_k) {
        ...
        STABLE_TORCH_CHECK(cu_seqlens_k.dtype() == torch::standalone::kInt32, "cu_seqlens_k must have dtype torch.int32");
        STABLE_TORCH_CHECK(max_seqlen_k_.has_value(), "max_seqlen_k must be provided if cu_seqlens_k is provided");
        STABLE_TORCH_CHECK(!paged_KV, "If cu_seqlens_k is passed in, then page table is not supported");
        STABLE_TORCH_CHECK(!kv_batch_idx_.has_value(), "If cu_seqlens_k is passed in, then page table is not supported");
    }
    ...
    if (!kv_batch_idx_.has_value()) {
        STABLE_TORCH_CHECK(batch_size == batch_size_k, "batch_size must be equal to batch_size_k");
    }
    int const max_headdim = get_max_headdim();
    STABLE_TORCH_CHECK(head_size <= max_headdim, "FlashAttention forward only supports head dimension at most " + std::to_string(max_headdim));
    STABLE_TORCH_CHECK(num_heads % num_heads_k == 0, "Number of heads in key/value must divide number of heads in query");
    if (head_size_v != head_size) {
        STABLE_TORCH_CHECK((head_size > 128 && head_size <= 192 && head_size_v > 96 && head_size_v <= 128) ||
                   ...
                   "or (Q/K <= 64 and V <= 512).");
        STABLE_TORCH_CHECK(dprops->major == 9, "Only Hopper supports different V headdim");
        if (head_size_v > 256) {
            STABLE_TORCH_CHECK(q_type == ScalarType::Half || q_type == ScalarType::BFloat16,
    ...
    if (seqused_q_.has_value()){
        ...
        STABLE_TORCH_CHECK(seqused_q.dtype() == torch::standalone::kInt32, "seqused_q must have dtype int32");
        CHECK_DEVICE(seqused_q); CHECK_CONTIGUOUS(seqused_q);
        CHECK_SHAPE(seqused_q, batch_size);
    }
    if (seqused_k_.has_value()) {
        auto seqused_k = seqused_k_.value();
        STABLE_TORCH_CHECK(seqused_k.dtype() == torch::standalone::kInt32, "seqused_k must have dtype int32");
    ...
    if (leftpad_k_.has_value()) {
        ...
        STABLE_TORCH_CHECK(leftpad_k.dtype() == torch::standalone::kInt32, "leftpad_k must have dtype int32");
    ...
    bool const is_varlen = is_varlen_q || is_varlen_k || seqused_q_.has_value() || seqused_k_.has_value() || leftpad_k_.has_value();
        ...
        STABLE_TORCH_CHECK(!is_varlen, "This flash attention build does not support varlen.");
    #endif

    int const alignment = q_type == torch::standalone::kFloat8_e4m3fn ? 16 : 8;
    STABLE_TORCH_CHECK(head_size % alignment == 0, "head_size should be a multiple of " + std::to_string(alignment));
    STABLE_TORCH_CHECK(head_size_v % alignment == 0, "head_size_v should be a multiple of " + std::to_string(alignment));
    ...
    if (out_.has_value()) {
        out = out_.value();
        STABLE_TORCH_CHECK(out.scalar_type() == out_type, "For FP16/BF16 input, output must have the same dtype as inputs. For FP8 input, output must have dtype BF16");
        CHECK_DEVICE(out);
        STABLE_TORCH_CHECK(out.stride(-1) == 1, "Output tensor must have contiguous last dimension");
        if (!is_varlen_q) {
        ...
        out = !is_varlen_q
            ? torch::stable::empty({batch_size, seqlen_q, num_heads, head_size_v}, opts.dtype(out_type))
            : torch::stable::empty({total_q, num_heads, head_size_v}, opts.dtype(out_type));
    }
    ...
    // Otherwise the kernel will be launched from cuda:0 device
    // Cast to char to avoid compiler warning about narrowing
    CUDAGuard device_guard{(char)q.get_device()};

    Tensor softmax_lse;
    if (!is_varlen_q) {
        softmax_lse = torch::stable::empty({batch_size, num_heads, seqlen_q}, opts.dtype(torch::standalone::kFloat));
    } else {
        softmax_lse = torch::stable::empty({num_heads, total_q}, opts.dtype(torch::standalone::kFloat));
    }
    ...
    if (k_new_.has_value()) {  // This needs to be set before get_pagedkv_tma
        Tensor k_new, v_new;
        STABLE_TORCH_CHECK(v_new_.has_value(), "If k_new is supplied, v_new must also be passed in");
        STABLE_TORCH_CHECK(seqused_k_.has_value(), "If k_new is supplied, seqlens_k must also be passed in");
        STABLE_TORCH_CHECK(seqlen_q <= seqlen_k, "If k_new is supplied, it must have seqlen <= the seqlen of the KV cache");
        ...
        if (is_varlen_k_new) {
            ...
            STABLE_TORCH_CHECK(cu_seqlens_k_new.dtype() == torch::standalone::kInt32, "cu_seqlens_k_new must have dtype torch.int32");
        }
        k_new = k_new_.value();
        v_new = v_new_.value();
        STABLE_TORCH_CHECK(k_new.dtype() == q_type, "k_new must have the same dtype as query");
        STABLE_TORCH_CHECK(v_new.dtype() == q_type, "v_new must have the same dtype as query");
        CHECK_DEVICE(k_new); CHECK_DEVICE(v_new);
        STABLE_TORCH_CHECK(k_new.stride(-1) == 1, "k_new tensor must have contiguous last dimension");
        STABLE_TORCH_CHECK(v_new.stride(-1) == 1, "v_new tensor must have contiguous last dimension");
    ...
    if (scheduler_needs_semaphore || use_dynamic_split) {
        ...
        if (scheduler_metadata_.has_value()) {
            ...
            STABLE_TORCH_CHECK(scheduler_metadata.dtype() == torch::standalone::kInt32, "scheduler_metadata must have dtype int32");
            tile_count_semaphore = scheduler_metadata;
        } else {
            tile_count_semaphore = torch::stable::empty({metadata_size}, opts.dtype(torch::standalone::kInt32));
        }
        if (scheduler_needs_semaphore && !use_dynamic_split) {
    ...
    if (q_v_.has_value()) {
        STABLE_TORCH_CHECK(head_size <= 64, "q_v is only supported for head_size <= 64");
        STABLE_TORCH_CHECK(q_type == ScalarType::Half || q_type == ScalarType::BFloat16,
                    "q_v is only supported for fp16 and bf16 data type");
        STABLE_TORCH_CHECK(params.arch == 90, "q_v is only supported for Hopper GPUs");
        Tensor q_v = q_v_.value();
        STABLE_TORCH_CHECK(q_v.dtype() == q_type, "q_v must have the same dtype as query");
        CHECK_DEVICE(q_v);
        STABLE_TORCH_CHECK(q_v.stride(-1) == 1, "q_v tensor must have contiguous last dimension");
        if (!is_varlen_q) {
    ...
    if (rotary_cos_.has_value()) {
        STABLE_TORCH_CHECK(k_new_.has_value(), "If rotary cos/sin are provided, new key / value to be appended to KV cache must also be provided");
        auto rotary_cos = rotary_cos_.value();
        CHECK_DEVICE(rotary_cos); CHECK_CONTIGUOUS(rotary_cos);
        params.rotary_dim = rotary_cos.size(1) * 2;
        STABLE_TORCH_CHECK(params.rotary_dim <= head_size, "rotary_dim must be <= headdim");
        STABLE_TORCH_CHECK(params.rotary_dim % 16 == 0, "Only rotary dimensions divisible by 16 are currently supported");
        const int seqlen_ro = rotary_cos.size(0);
        if (paged_KV) {
            STABLE_TORCH_CHECK(seqlen_ro >= seqlen_k, "cos/sin seqlen must be at least the seqlen of KV cache");
        }
        CHECK_SHAPE(rotary_cos, seqlen_ro, params.rotary_dim / 2);
        STABLE_TORCH_CHECK(rotary_cos.scalar_type() == q_type, "rotary_cos must have the same dtype as query");

        STABLE_TORCH_CHECK(rotary_sin_.has_value(), "If rotary cos is provided, rotary sin must also be provided");
        auto rotary_sin = rotary_sin_.value();
        CHECK_DEVICE(rotary_sin); CHECK_CONTIGUOUS(rotary_sin);
        CHECK_SHAPE(rotary_sin, seqlen_ro, params.rotary_dim / 2);
        STABLE_TORCH_CHECK(rotary_sin.scalar_type() == q_type, "rotary_cos must have the same dtype as query");
        ...
        if (seqlens_rotary_.has_value()) {
            ...
            STABLE_TORCH_CHECK(seqlens_rotary.dtype() == torch::standalone::kInt32, "seqlens_rotary must have dtype torch.int32");
    ...
    if (kv_batch_idx_.has_value()) {
        ...
        STABLE_TORCH_CHECK(kv_batch_idx.scalar_type() == torch::standalone::kInt32, "kv_batch_idx must have dtype int32");
    ...
    if (params.num_splits > 1) {
        STABLE_TORCH_CHECK(params.num_splits <= 256, "num_splits > 256 not supported");
        if (!is_varlen_q) {
            out_accum = torch::stable::empty({params.num_splits, batch_size, num_heads, seqlen_q, head_size_v}, opts.dtype(outaccum_type));
            softmax_lse_accum = torch::stable::empty({params.num_splits, batch_size, num_heads, seqlen_q}, opts.dtype(torch::standalone::kFloat));
            params.oaccum_batch_stride = out_accum.stride(1);
            params.lseaccum_batch_stride = softmax_lse_accum.stride(1);
        } else {
            out_accum = torch::stable::empty({params.num_splits, num_heads, total_q, head_size_v}, opts.dtype(outaccum_type));
            softmax_lse_accum = torch::stable::empty({params.num_splits, num_heads, total_q}, opts.dtype(torch::standalone::kFloat));
        }
    ...
    STABLE_TORCH_CHECK(params.softcap == 0.0, "This flash attention build does not support tanh softcapping.");
    #endif
    #ifdef FLASHATTENTION_DISABLE_SPLIT
    STABLE_TORCH_CHECK(params.num_splits == 1, "This flash attention build does not support splits.");
    #endif
    #ifdef FLASHATTENTION_DISABLE_PACKGQA
    STABLE_TORCH_CHECK(!params.pack_gqa || params.arch < 90 || (params.page_table && !params.pagedkv_tma) || params.num_splits > 1, "This flash attention build does not support pack_gqa.");
    #endif
    #ifdef FLASHATTENTION_DISABLE_PAGEDKV
    STABLE_TORCH_CHECK(!(params.page_table && !params.pagedkv_tma), "This flash attention build does not support paged KV.");
    #endif
    #ifdef FLASHATTENTION_DISABLE_APPENDKV
    STABLE_TORCH_CHECK(!k_new_.has_value(), "This flash attention build does not support appending KV.");
    #endif
    ...
    if (total_q > 0 && (total_k + params.total_knew) > 0 && num_heads_k > 0) {
        ...
        if (params.num_splits > 1) {
            ...
            tile_count_semaphore.index({torch::stable::indexing::Slice(0, 1)}).zero_();
        }
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor> mha_bwd(
    ...
    int64_t sm_margin
        ...
        STABLE_TORCH_CHECK(false, "This flash attention build does not support backward.");
    #endif
    ...
    auto q_type = q.dtype();
    STABLE_TORCH_CHECK(q_type == torch::standalone::kFloat16 || q_type == torch::standalone::kBFloat16,
                "FlashAttention only support fp16 and bf16 data type");
    STABLE_TORCH_CHECK(k.dtype() == q_type, "query and key must have the same dtype");
    STABLE_TORCH_CHECK(v.dtype() == q_type, "query and value must have the same dtype");
    STABLE_TORCH_CHECK(out.dtype() == q_type, "query and out must have the same dtype");
    STABLE_TORCH_CHECK(dout.dtype() == q_type, "query and dout must have the same dtype");
    ...
    STABLE_TORCH_CHECK(k.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    STABLE_TORCH_CHECK(v.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    STABLE_TORCH_CHECK(out.stride(-1) == 1, "out tensor must have contiguous last dimension");
    STABLE_TORCH_CHECK(dout.stride(-1) == 1, "dout tensor must have contiguous last dimension");
    ...
    int const total_k = !is_varlen_k ? batch_size * k.size(1) : k.size(0);
    int const num_heads_k = k.size(-2);
    STABLE_TORCH_CHECK(head_size % 8 == 0, "head_size should be a multiple of 8");
    STABLE_TORCH_CHECK(head_size_v % 8 == 0, "head_size_v should be a multiple of 8");
    int const max_headdim = get_max_headdim();
    STABLE_TORCH_CHECK(std::max(head_size, head_size_v) <= max_headdim, "FlashAttention forward only supports head dimension at most " + std::to_string(max_headdim));
    STABLE_TORCH_CHECK(num_heads % num_heads_k == 0, "Number of heads in key/value must divide number of heads in query");
    ...
    if (dq_.has_value()) {
        dq = dq_.value();
        STABLE_TORCH_CHECK(dq.dtype() == q_type, "dq must have the same dtype as q");
        CHECK_DEVICE(dq);
        STABLE_TORCH_CHECK(dq.stride(-1) == 1, "dq must have contiguous last dimension");
        if (!is_varlen_q) {
    ...
    if (dk_.has_value()) {
        dk = dk_.value();
        STABLE_TORCH_CHECK(dk.dtype() == q_type, "dk must have the same dtype as q");
        CHECK_DEVICE(dk);
        STABLE_TORCH_CHECK(dk.stride(-1) == 1, "dk must have contiguous last dimension");
        if (!is_varlen_k) {
    ...
    if (dv_.has_value()) {
        dv = dv_.value();
        STABLE_TORCH_CHECK(dv.dtype() == q_type, "dv must have the same dtype as q");
        CHECK_DEVICE(dv);
        STABLE_TORCH_CHECK(dv.stride(-1) == 1, "dv must have contiguous last dimension");
        if (!is_varlen_k) {
    ...
    if (!is_varlen) {
        ...
        softmax_d = torch::stable::empty({batch_size, num_heads, seqlen_q_rounded}, opts.dtype(torch::standalone::kFloat));
        softmax_lse_log2 = torch::stable::empty({batch_size, num_heads, seqlen_q_rounded}, opts.dtype(torch::standalone::kFloat));
    } else {
        softmax_d = torch::stable::empty({num_heads, total_q_padded_rounded}, opts.dtype(torch::standalone::kFloat));
        softmax_lse_log2 = torch::stable::empty({num_heads, total_q_padded_rounded}, opts.dtype(torch::standalone::kFloat));
    }
    Tensor dq_accum, dk_accum, dv_accum;
    if (!is_varlen) {
        dq_accum = torch::stable::empty({batch_size, num_heads, seqlen_q_rounded * head_size_rounded}, opts.dtype(torch::standalone::kFloat));
    } else {
        dq_accum = torch::stable::empty({num_heads, total_q_padded_rounded * head_size_rounded}, opts.dtype(torch::standalone::kFloat));
    }
    if (num_heads_k != num_heads) {  // MQA / GQA
        if (!is_varlen) {
            dk_accum = torch::zeros({batch_size, num_heads_k, seqlen_k_rounded * head_size_rounded}, opts.dtype(torch::standalone::kFloat));
            dv_accum = torch::zeros({batch_size, num_heads_k, seqlen_k_rounded * head_size_v_rounded}, opts.dtype(torch::standalone::kFloat));
        } else {
            dk_accum = torch::zeros({num_heads_k, total_k_padded_rounded, head_size_rounded}, opts.dtype(torch::standalone::kFloat));
            dv_accum = torch::zeros({num_heads_k, total_k_padded_rounded, head_size_v_rounded}, opts.dtype(torch::standalone::kFloat));
        }
    }
    ...
    // auto tile_count_semaphore = (params.is_causal || params.is_local) ? torch::zeros({1}, opts.dtype(torch::standalone::kInt32)) : torch::stable::empty({1}, opts.dtype(torch::standalone::kInt32));
    // params.tile_count_semaphore = tile_count_semaphore.data_ptr<int>();
    // Will be zero'ed out in the backward preprocess kernel
    Tensor dq_semaphore = torch::stable::empty({(seqlen_q + kBlockM - 1) / kBlockM, batch_size, num_heads}, opts.dtype(torch::standalone::kInt32));
    params.dq_semaphore = dq_semaphore.data_ptr<int>();
    if (num_heads_k != num_heads && params.deterministic) {
        // TODO: do we need to zero them out?
        Tensor dk_semaphore = torch::stable::empty({(seqlen_k + kBlockN - 1) / kBlockN, batch_size, num_heads_k}, opts.dtype(torch::standalone::kInt32));
        Tensor dv_semaphore = torch::stable::empty({(seqlen_k + kBlockN - 1) / kBlockN, batch_size, num_heads_k}, opts.dtype(torch::standalone::kInt32));
...
mha_combine(Tensor out_partial,         // num_splits x batch_size x seqlen x num_heads x head_size
    ...
    auto dprops = getCurrentDeviceProperties();
    bool is_sm8x = dprops->major >= 8;
    STABLE_TORCH_CHECK(is_sm8x, "Attention combine function only supports Ampere GPUs or newer.");

    auto out_partial_type = out_partial.scalar_type();
    STABLE_TORCH_CHECK(out_partial_type == ScalarType::Float, "Attention combine function only support fp32 data type");
    STABLE_TORCH_CHECK(lse_partial.scalar_type() == ScalarType::Float, "Attention combine function only support fp32 data type");

    CHECK_DEVICE(out_partial); CHECK_DEVICE(lse_partial);

    STABLE_TORCH_CHECK(out_partial.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    STABLE_TORCH_CHECK(lse_partial.stride(-2) == 1, "LSE tensor must be contiguous in the seqlen dimension");
    ...
    STABLE_TORCH_CHECK(num_splits <= 256, "FlashAttention combine only supports num_splits at most 256");
    ...
    auto pad = [](Tensor x, int alignment) {
        return x.size(-1) % alignment == 0 ? x : torch::stable::nn::functional::pad(x, torch::standalone::nn::functional::PadFuncOptions({0, alignment - x.size(-1) % alignment}));
    };
    ...
    STABLE_TORCH_CHECK(out_type == ScalarType::Float || out_type == ScalarType::BFloat16 || out_type == ScalarType::Half, "Output type must be FP32, FP16 or BF16");
    Tensor out;
    if (out_.has_value()) {
        out = out_.value();
        STABLE_TORCH_CHECK(out.scalar_type() == out_type);
        CHECK_DEVICE(out);
        STABLE_TORCH_CHECK(out.stride(-1) == 1, "Output tensor must have contiguous last dimension");
        CHECK_SHAPE(out, batch_size, seqlen, num_heads, head_size_og);
        if (head_size_og % alignment != 0) {
            out = torch::stable::empty({batch_size, seqlen, num_heads, head_size}, opts.dtype(out_type));
        }
    } else {
        out = torch::stable::empty({batch_size, seqlen, num_heads, head_size}, opts.dtype(out_type));
    }
    ...
    // Otherwise the kernel will be launched from cuda:0 device
    // Cast to char to avoid compiler warning about narrowing
    CUDAGuard device_guard{(char)out_partial.get_device()};

    auto softmax_lse = torch::stable::empty({batch_size, num_heads, seqlen}, opts.dtype(torch::standalone::kFloat)).transpose(1, 2);
    ...
    if (head_size_og % alignment != 0) {
        out = out.index({"...", torch::stable::indexing::Slice(torch::stable::indexing::None, head_size_og)});
