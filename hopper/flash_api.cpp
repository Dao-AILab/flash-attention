...
mha_fwd_kvcache(at::Tensor &q,   // batch_size x seqlen_q x num_heads x head_size or total_q x num_heads x head_size
    ...
    if (num_splits <= 0) {
        ...
        auto [kBlockM, kBlockN, IntraWGOverlap] = tile_size_fwd(head_size_rounded, is_causal_or_local, torch::elementSize(q_type) /*element_size*/, false /*v_colmajor*/, page_table_.has_value());
