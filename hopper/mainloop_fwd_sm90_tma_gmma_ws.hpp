...
template <typename Ktraits, bool Is_causal> struct CollectiveMainloopFwd {
  ...
  using SmemLayoutK = decltype(
      tile_to_shape(SmemLayoutAtomK{},
                    make_shape(shape<1>(TileShape_MNK{}),
                               shape<2>(TileShape_MNK{}), Int<kStages>{})));

  using SmemLayoutAtomVFp8 =
      decltype(cutlass::gemm::collective::detail::ss_smem_selector<
               GMMA::Major::K, Element, decltype(cute::get<2>(TileShape_MNK{})),
               decltype(cute::get<1>(TileShape_MNK{}))>());
  using SmemLayoutVFp8 = decltype(
      tile_to_shape(SmemLayoutAtomVFp8{},
                    make_shape(shape<2>(TileShape_MNK{}),
                               shape<1>(TileShape_MNK{}), Int<kStages>{})));

  using SmemLayoutVFp16 = SmemLayoutK;
  // Note this is the transpose in terms of the view, not in terms of memory.
  using SmemLayoutVtFp16 = decltype(cute::composition(
      SmemLayoutVFp16{},
      make_layout(make_shape(get<2>(TileShape_MNK{}), get<1>(TileShape_MNK{}),
                             Int<kStages>{}),
                  make_stride(get<1>(TileShape_MNK{}), _1{},
                              Int<size(SmemLayoutVFp16{}(_, _, _0{}))>{}))));

  using SmemLayoutV = decltype(
      cute::conditional_return<is_same_v<Element, cutlass::float_e4m3_t>>(
          SmemLayoutVFp8{}, SmemLayoutVFp16{}));
  using SmemLayoutVt = decltype(
      cute::conditional_return<is_same_v<Element, cutlass::float_e4m3_t>>(
          SmemLayoutVFp8{}, SmemLayoutVtFp16{}));

  // Dummy S layout for getting the shape for GEMM-II.
  using SmemLayoutAtomS =
      decltype(cutlass::gemm::collective::detail::ss_smem_selector<
               GMMA::Major::K, Element, decltype(cute::get<0>(TileShape_MNK{})),
               decltype(cute::get<1>(TileShape_MNK{}))>());
  using SmemLayoutS = decltype(
      tile_to_shape(SmemLayoutAtomS{}, make_shape(shape<0>(TileShape_MNK{}),
  ...
  //                            make_shape(shape<2>(TileShape_MNK{}),
  //                            shape<1>(TileShape_MNK{}), Int<kStages>{}),
  //                            Step<_2, _1, _3>{}));  // This gives correct
  //                            results, without Step it's wrong
  // using SmemLayoutAtomVt =
  // decltype(cutlass::gemm::collective::detail::ss_smem_selector<GMMA::Major::MN,
  // Element,
  //     decltype(cute::get<2>(TileShape_MNK{})),
  //     decltype(cute::get<1>(TileShape_MNK{}))>());
  // using SmemLayoutVt =
  ...
  using TMA_KV = decltype(make_tma_copy(
      ...
      size<0>(ClusterShape{}))); // mcast along M mode for this N load, if any
                                 //
  using TileShapeVFP8 = decltype(
      make_shape(cute::get<2>(TileShape_MNK{}), cute::get<1>(TileShape_MNK{})));
  using TileShapeVFP16 = decltype(
      make_shape(cute::get<1>(TileShape_MNK{}), cute::get<2>(TileShape_MNK{})));
  using TileShapeV = decltype(
      cute::conditional_return<is_same_v<Element, cutlass::float_e4m3_t>>(
          TileShapeVFP8{}, TileShapeVFP16{}));
  using TMA_VFP8 = decltype(make_tma_copy(
      GmemTiledCopyKV{},
      make_tensor(make_gmem_ptr(static_cast<Element const *>(nullptr)),
                  repeat_like(StrideQKV{}, int32_t(0)), StrideQKV{}),
      take<0, 2>(SmemLayoutV{}), TileShapeV{},
      size<0>(ClusterShape{}))); // mcast along M mode for this N load, if any
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    ...
    auto gmemLayoutV =
        cute::conditional_return<is_same_v<Element, cutlass::float_e4m3_t>>(
            gmemLayoutVFp8, gmemLayoutVFp16);
    Tensor mV =
        make_tensor(make_gmem_ptr(args.ptr_V), gmemLayoutV, args.stride_V);
    TMA_V tma_load_V = make_tma_copy(
        GmemTiledCopyKV{}, mV, SmemLayoutV{}(_, _, _0{}),
        cute::conditional_return<is_same_v<Element, cutlass::float_e4m3_t>>(
            select<2, 1>(TileShape_MNK{}), select<1, 2>(TileShape_MNK{})),
        size<0>(ClusterShape{})); // mcast along M mode for this N load, if any
    return {args.shape_Q,
            args.shape_K,
            cutlass::FastDivmod(
  ...
  load(Params const &mainloop_params, MainloopPipeline pipeline_k,
    ...
    Tensor mQ =
        mainloop_params.tma_load_Q.get_tma_tensor(mainloop_params.shape_Q);
    Tensor mK =
        mainloop_params.tma_load_K.get_tma_tensor(mainloop_params.shape_K);
    ...
    Tensor gK =
        local_tile(mK(_, _, bidh_kv, bidb), select<1, 2>(TileShape_MNK{}),
                   make_coord(_, _0{})); // (N, K, _)
    Tensor gV = local_tile(
        mV(_, _, bidh_kv, bidb), TileShapeV{},
        cute::conditional_return<is_same_v<Element, cutlass::float_e4m3_t>>(
            make_coord(_0{}, _), make_coord(_, _0{}))); // (N, K, _)
