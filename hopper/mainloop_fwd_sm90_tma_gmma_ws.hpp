...
struct CollectiveMainloopFwd {
    ...
    template <typename Scheduler, typename SharedStorage>
    CUTLASS_DEVICE void
    load_fp8_ver1(Params const& mainloop_params,
         MainloopPipeline pipeline_k,
         MainloopPipeline pipeline_v,
        ...
        for (; n_block >= 0; --n_block) {
            ...
            pipeline_vt.producer_acquire(smem_pipe_write_v);
            do_transpose_V(smem_pipe_read_v.index());
            pipeline_vt.producer_commit(smem_pipe_write_v);
    ...
    load_fp8(Params const& mainloop_params,
        ...
        // NOTE: for fp8 can replace with NamedBarrier.           
    
#ifndef NO_UNION
    #ifdef NEW_FP8_EPI_BARRIER
        cutlass::arch::NamedBarrier::sync(NumMmaThreads + cutlass::NumThreadsPerWarpGroup, static_cast<int>(FwdNamedBarriers::OutEmpty) /*id*/);
    #else
        shared_storage.barrier_O.wait((work_idx + 1) % 2);
    #endif
#endif
        ...
        // used with dynamic tile scheduler (causal)
        scheduler.prefetch_next_work(scheduler_params, work_tile_info);
    ...
    mma_init_fp8() {        
        ...
        // Tell producer (warpgroup 0) that smem_q is ready
    ...
    #ifdef NEW_FP8_EPI_BARRIER
        // For fp8, use NamedBarrier::OutEmpty for epilogue sync
        cutlass::arch::NamedBarrier::arrive(NumMmaThreads + cutlass::NumThreadsPerWarpGroup, static_cast<int>(FwdNamedBarriers::OutEmpty) /*id*/);
    #endif

        // cutlass::arch::NamedBarrier::arrive(NumMmaThreads + cutlass::NumThreadsPerWarpGroup, static_cast<int>(FwdNamedBarriers::TransposeVProducer) /*id*/);
    ...
    mma_fp8(Params const& mainloop_params,
        ...
        // Allocate accumulator for S
        Tensor tSrS = partition_fragment_C(tiled_mma0, select<0, 1>(TileShape_MNK{}));
        // first consumer wait for K
        consumer_wait(pipeline_k, smem_pipe_read); // wait K
        
        warp_scheduler_barrier_sync(); // used for pingpong
        flash::gemm</*zero_init=*/true, /*wg_wait=*/-1>(tiled_mma0, tSrQ, tSrK(_, _, _, smem_pipe_read.index()), tSrS);
    #ifndef NO_UNION    
        warp_scheduler_barrier_arrive(); // used for pingpong        
        // overlap first gemm with prior epilogue        
        if (work_idx != 0) {
        #ifdef NEW_FP8_EPI_BARRIER
            tma_store_wait<0>();
            cutlass::arch::NamedBarrier::arrive(NumMmaThreads + cutlass::NumThreadsPerWarpGroup, static_cast<int>(FwdNamedBarriers::OutEmpty) /*id*/);
...
#ifdef USE_CUSTOM_SOFTMAX
        softmax.template online_softmax_and_rescale_o</*Is_first=*/true>(tSrS, tOrO, mainloop_params.softmax_scale_log2);
#else
        softmax.template online_softmax</*Is_first=*/true>(tSrS, mainloop_params.softmax_scale_log2);
#endif        
        // for fp8 use different layout reshape
        Tensor tOrP = make_tensor(convert_type<Element>(tSrS).data(), convert_layout_acc_Aregs_fp8(tSrS.layout()));
        permute_regs_A_to_C(tOrP);
        
#ifndef USE_CUSTOM_SOFTMAX
        Tensor scores_scale = make_fragment_like(softmax.row_max);
        clear(scores_scale);
#endif

        consumer_wait(pipeline_vt, smem_pipe_read); // wait V
        flash::gemm</*zero_init=*/true, /*wg_wait=*/0>(tiled_mma1, tOrP, tOrV(_, _, _, smem_pipe_read.index()), tOrO);        

    #ifndef RELEASE_PATTERN
        pipeline_vt.consumer_release(smem_pipe_read);  // release current V
    #endif
        
        ++smem_pipe_read; // advance pipeline state read
        // ++smem_pipe_release; // advance pipeline state release (if not staggering)
        --n_block;
        ...
        for (int iter = 0; iter < extra_iterations && n_block >= 0 ; ++iter) {
            ...
            pipeline_k.consumer_release(smem_pipe_release); // release previous K            
        #endif
            consumer_wait(pipeline_k, smem_pipe_read); // wait K
            warp_scheduler_barrier_sync(); // used for pingpong
        ...
        #ifdef USE_CUSTOM_SOFTMAX
            softmax.template online_softmax_and_rescale_o</*Is_first=*/false>
                (tSrS, tOrO, mainloop_params.softmax_scale_log2);
        #else    
            cute::copy(softmax.template max</*Is_first=*/false>(tSrS, mainloop_params.softmax_scale_log2), scores_scale);            
            softmax.rescale_o(tOrO, scores_scale);
            softmax.template online_softmax</*Is_first=*/false>(tSrS, mainloop_params.softmax_scale_log2);            
        ...
        #ifdef RELEASE_PATTERN                 
            pipeline_vt.consumer_release(smem_pipe_release);  // release previous V
        #endif
            consumer_wait(pipeline_vt, smem_pipe_read); // wait V              
            ...
            ++smem_pipe_read; // advance pipeline state read
        #ifdef RELEASE_PATTERN    
            ++smem_pipe_release; // advance pipeline state release
        #endif
        ...
        // Tell warpgroup 0 that smem_q is ready
        cutlass::arch::NamedBarrier::arrive(NumMmaThreads + cutlass::NumThreadsPerWarpGroup, static_cast<int>(FwdNamedBarriers::QueryEmpty) /*id*/);
...
#ifdef RELEASE_PATTERN
        pipeline_k.consumer_release(smem_pipe_release);  // release end K
        pipeline_vt.consumer_release(smem_pipe_release);  // release end V
        ++smem_pipe_release; // advance pipeline state release
#endif
        return;
    ...
    template <typename SharedStorage, typename FrgTensorO, typename Softmax>
    CUTLASS_DEVICE void
    mma_fp8_ver1(Params const& mainloop_params,
        MainloopPipeline pipeline_k,
        MainloopPipelineNoTMA pipeline_vt,
        PipelineState& smem_pipe_read_k,
        PipelineState& smem_pipe_read_vt,      
        FrgTensorO& tOrO,
        Softmax& softmax,
        int n_block_count,
        int thread_idx,
        ...
        Tensor tSrS = partition_fragment_C(tiled_mma0, select<0, 1>(TileShape_MNK{}));
        // first consumer wait for K
        consumer_wait(pipeline_k, smem_pipe_read_k); // wait K
        ...
        warpgroup_wait<0>();
        warp_scheduler_barrier_arrive(); // used for pingpong          
        pipeline_k.consumer_release(smem_pipe_read_k); // release current K          
        ++smem_pipe_read_k;
        ...
        //     softmax.rescale_o(tOrO, scores_scale);
        //     consumer_wait(pipeline_vt, smem_pipe_read_vt); // wait V
        //     warp_scheduler_barrier_sync(); // used for pingpong     
        //     flash::gemm</*zero_init=*/false, /*wg_wait=*/0>(tiled_mma1, tOrP, tOrV(_, _, _, smem_pipe_read_vt.index()), tOrO);              
        //     warp_scheduler_barrier_arrive(); // used for pingpong
        //     pipeline_vt.consumer_release(smem_pipe_read_vt);  // release current V
        ...
        //     ++smem_pipe_read_k; // advance pipeline state read        
        //     ++smem_pipe_read_vt; // advance pipeline state read                            
            
        //     cute::copy(make_tensor(convert_type<Element>(tSrS).data(),
        //                            convert_layout_acc_Aregs_fp8(tSrS.layout())), tOrP);                
        //     permute_regs_A_to_C(tOrP);
        // }
        ...
        for (; n_block > 0; --n_block) {                                
            ...
            softmax.rescale_o(tOrO, scores_scale);
            consumer_wait(pipeline_vt, smem_pipe_read_vt); // wait V
            flash::gemm</*zero_init=*/false, /*wg_wait=*/-1>(tiled_mma1, tOrP, tOrV(_, _, _, smem_pipe_read_vt.index()), tOrO);              
            warp_scheduler_barrier_arrive(); // used for pingpong
            warpgroup_wait<1>();
        ...
        permute_regs_A_to_C(tOrP);
        softmax.rescale_o(tOrO, scores_scale);
        consumer_wait(pipeline_vt, smem_pipe_read_vt);
        flash::gemm</*zero_init=*/false, /*wg_wait=*/-1>(tiled_mma1, tOrP, tOrV(_, _, _, smem_pipe_read_vt.index()), tOrO);
        
        cute::copy(softmax.template finalize</*Check_inf=*/Is_causal>(tSrS, mainloop_params.softmax_scale_log2), scores_scale);
        warpgroup_wait<0>();
        pipeline_vt.consumer_release(smem_pipe_read_vt);  // release V, otherwise producers will hang
        ++smem_pipe_read_vt;
