...
// Element_>
template <typename Ktraits> struct CollectiveEpilogueFwd {

  using PrecType = typename Ktraits::Element;
  using Element = decltype(
      cute::conditional_return<is_same_v<PrecType, cutlass::float_e4m3_t>>(
          cutlass::half_t{}, PrecType{}));
  ...
  using StrideLSE =
      cute::Stride<_1, int64_t, int64_t>; // (seqlen_q, head, batch)
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    ...
    return {args.ptr_O,   args.shape_O,    args.stride_O,
            args.ptr_LSE, args.stride_LSE, tma_store_O};
  }
  ...
  store(Params const &epilogue_params, FrgTensorO const &tOrO,
    ...
    Tensor mO =
        epilogue_params.tma_store_O.get_tma_tensor(epilogue_params.shape_O);
    ...
    Tensor gLSE = local_tile(mLSE(_, bidh, bidb), Shape<Int<kBlockM>>{},
                             make_coord(m_block));
...
#pragma unroll
  ...
  CUTLASS_DEVICE void
  store_zero(Params const &epilogue_params, int thread_idx,
             cute::tuple<int32_t, int32_t, int32_t> const &block_coord) {
    auto [m_block, bidh, bidb] = block_coord;
