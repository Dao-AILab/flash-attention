  ...
  if (warp_group_idx == 0) { // Producer
    ...
    if (warp_idx_in_warpgroup == 0) { // Load Q, K, V
      ...
      for (auto work_tile_info = scheduler.get_initial_work();
        ...
        int n_block_max =
            collective_mainloop.get_n_block_max(mainloop_params, m_block);
    ...
    for (auto work_tile_info = scheduler.get_initial_work();
      ...
      if (Is_causal &&
        ...
        collective_epilogue.store_zero(
            epilogue_params, threadIdx.x - NumCopyThreads, block_coord);
        continue;
      }
      ...
      collective_epilogue.store(epilogue_params, tOrO, softmax.row_sum,
                                shared_storage, tiled_mma1,
