...
def blockdiag_to_dense_mlp_bert(state_dict):
    ...
    names = {
        ...
        if re.match("bert.encoder.layer.(\d+).(mlp.fc(1|2)|(intermediate|output).dense).weight", name)
    }
    for name in names:
...
def interpolate_pos_embedding(state_dict, out_seqlen, pos_embedding_name="model.pos_encoder.pe", interleave=False):
    ...
    if interleave:
        ...
        emb_square_expanded = emb_square.repeat_interleave(math.isqrt(reps[1]), axis=1).repeat_interleave(
            math.isqrt(reps[1]), axis=2
        )
    ...
    ret = {k: v for k, v in ret.items() if not k.endswith("inner_attn.layout")}
    return ret
