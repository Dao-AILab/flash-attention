...
from src.utils.flops import has_deepspeed_profiling, has_fvcore_profiling, profile_deepspeed, profile_fvcore
...
class FlopCount(Callback):
    ...
    def __init__(
        self,
        profilers: List[str] = ["fvcore", "deepspeed"],
    ...
    def on_fit_start(self, trainer: Trainer, pl_module: LightningModule) -> None:
        if "fvcore" in self.profilers:
            _, macs, _, acts = profile_fvcore(
                pl_module.to(self.device), input_size=self.input_size, input_dtype=self.input_dtype, detailed=True
            )
