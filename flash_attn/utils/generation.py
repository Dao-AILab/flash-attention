...
def decode(
    ...
    if cg:
        ...
        model._decoding_cache = update_graph_cache(
            model, model._decoding_cache, batch_size, seqlen_og, max_length, tensor_parallel=tensor_parallel
        )
    ...
    with torch.inference_mode():
        ...
        while True:
            position_ids = torch.full(
                (batch_size, 1), inference_params.sequence_len_offset, dtype=torch.long, device=input_ids.device
            )
            if not cg:
                logits = model(
                    rearrange(next_token, "b -> b 1"), position_ids=position_ids, inference_params=inference_params
                ).logits[:, -1]
            else:
                logits = model._decoding_cache.run(
                    rearrange(next_token, "b -> b 1"), position_ids, inference_params.sequence_len_offset
                )
            if vocab_size is not None:
                logits = logits[..., :vocab_size]
            scores.append(logits if not cg else logits.clone())
    ...
    return output_cls(sequences=torch.cat([input_ids, torch.stack(sequences, dim=1)], dim=1), scores=tuple(scores))
...
def allocate_kv_cache(
    max_batch_size, max_seqlen, nheads, headdim, layers: Union[int, Sequence], device, dtype=torch.float16
):
...
def update_graph_cache(model, cache, batch_size, seqlen_og, max_seqlen, tensor_parallel=1, dtype=None, n_warmups=2):
    if cache is None:
    ...
    for s_type in range(seqlen_to_seqlen_type(seqlen_og), seqlen_to_seqlen_type(max_seqlen) + 1):
        if s_type not in cache.callables:
            ...
            cache.callables[s_type] = capture_graph(
                model, cache.inference_params, batch_size, max_seqlen_, mempool=cache.mempool, n_warmups=n_warmups
            )
...
def capture_graph(model, inference_params, batch_size, max_seqlen, mempool=None, n_warmups=2):
    ...
    with torch.cuda.stream(s):
        for _ in range(n_warmups):
            logits = model(input_ids, position_ids=position_ids, inference_params=inference_params).logits[:, -1]
        s.synchronize()
