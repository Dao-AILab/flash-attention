...
def _flash_attn_forward(
    q, k, v, dropout_p, softmax_scale, causal, max_past, return_softmax
):
    maybe_contiguous = lambda x: x.contiguous() if x.stride(-1) != 1 else x
    q, k, v = [maybe_contiguous(x) for x in (q, k, v)]
    out, q, k, v, out_padded, softmax_lse, S_dmask, rng_state = flash_attn_cuda.fwd(
        q, k, v, None, dropout_p, softmax_scale, causal, max_past, return_softmax, None
    )
...
class FlashAttnKVPackedFunc(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, kv, dropout_p, softmax_scale, causal, max_past, return_softmax):
        if softmax_scale is None:
    ...
    @staticmethod
    def forward(
        ctx, q, k, v, dropout_p, softmax_scale, causal, max_past, return_softmax
    ):
        if softmax_scale is None:
            softmax_scale = q.shape[-1] ** (-0.5)
        out, q, k, v, out_padded, softmax_lse, S_dmask, rng_state = _flash_attn_forward(
...
def flash_attn_kvpacked_func(
    ...
    return FlashAttnKVPackedFunc.apply(
        q, kv, dropout_p, softmax_scale, causal, max_past, return_attn_probs
    )
...
def flash_attn_func(
    ...
    return FlashAttnFunc.apply(
        q, k, v, dropout_p, softmax_scale, causal, max_past, return_attn_probs
    )
