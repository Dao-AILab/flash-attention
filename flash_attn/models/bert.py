...
def create_mlp_cls(config, layer_idx=None, return_residual=False):
    ...
    if fused_mlp:
        assert config.hidden_act in ["gelu_new", "gelu_fast"], "fused_mlp only " "supports approximate gelu"
    if not fused_mlp:
        ...
        mlp_cls = partial(
            FusedMLP, hidden_features=inner_dim, checkpoint_lvl=mlp_checkpoint_lvl, return_residual=return_residual
        )
    return mlp_cls
...
class BertEncoder(nn.Module):
    def __init__(self, config: BertConfig):
        ...
        self.layers = nn.ModuleList([create_block(config, layer_idx=i) for i in range(config.num_hidden_layers)])
    ...
    def forward(self, hidden_states, key_padding_mask=None, subset_mask=None):
        ...
        if key_padding_mask is None or not self.use_flash_attn:
            mixer_kwargs = {"key_padding_mask": key_padding_mask} if key_padding_mask is not None else None
            ...
            hidden_states, indices, cu_seqlens, max_seqlen_in_batch = unpad_input(hidden_states, key_padding_mask)
            ...
            if subset_mask is None:
                ...
                if key_padding_mask is not None:
                    ...
                    subset_cu_seqlens = F.pad(torch.cumsum(subset_seqlens, dim=0, dtype=torch.torch.int32), (1, 0))
                else:
...
class BertPredictionHeadTransform(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if not self.fused_dropout_add_ln:
            ...
            hidden_states = layer_norm(hidden_states, self.layer_norm.weight, self.layer_norm.bias, self.layer_norm.eps)
        return hidden_states
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, attention_mask=None, masked_tokens_mask=None):
        ...
        sequence_output = self.encoder(hidden_states, key_padding_mask=attention_mask, subset_mask=subset_mask)
        ...
        if labels is not None and next_sentence_label is not None:
            if self.dense_seq_output and labels is not None:  # prediction_scores are already flattened
                ...
                masked_lm_loss = self.mlm_loss(
                    rearrange(prediction_scores, "... v -> (...) v"), rearrange(labels, "... -> (...)")
                )
            next_sentence_loss = self.nsp_loss(
                rearrange(seq_relationship_score, "... t -> (...) t"), rearrange(next_sentence_label, "... -> (...)")
            )
...
def remap_state_dict(state_dict, config):
    ...
    def key_mapping_ln(key):
        ...
        key = re.sub(
            r"^bert.encoder.layers.(\d+).output.LayerNorm.(weight|bias)", r"bert.encoder.layers.\1.norm2.\2", key
        )
        key = re.sub(
            r"^cls.predictions.transform.LayerNorm.(weight|bias)", r"cls.predictions.transform.layer_norm.\1", key
        )
        return key
    ...
    def key_mapping_mlp(key):
        key = re.sub(
            r"^bert.encoder.layers.(\d+).intermediate.dense.(weight|bias)", r"bert.encoder.layers.\1.mlp.fc1.\2", key
        )
        key = re.sub(
            r"^bert.encoder.layers.(\d+).output.dense.(weight|bias)", r"bert.encoder.layers.\1.mlp.fc2.\2", key
        )
        return key
