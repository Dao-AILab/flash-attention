...
def remap_state_dict_hf_gpt_neox(state_dict, config):
    ...
    def key_mapping_attn(key):
        key = re.sub(r"^transformer.layers.(\d+).attention.dense.", r"transformer.layers.\1.mixer.out_proj.", key)
        key = re.sub(
            r"^transformer.layers.(\d+).attention.rotary_emb.", r"transformer.layers.\1.mixer.rotary_emb.", key
        )
        return key
