...
def remap_state_dict_hf_opt(state_dict, config):
    ...
    def key_mapping_emb(key):
        ...
        key = re.sub(r"^transformer.embed_positions.", "transformer.embeddings.position_embeddings.", key)
        return key
    ...
    def key_mapping_ln(key):
        ...
        key = re.sub(r"^transformer.layers.(\d+).self_attn_layer_norm.", r"transformer.layers.\1.norm1.", key)
        key = re.sub(r"^transformer.layers.(\d+).final_layer_norm.", r"transformer.layers.\1.norm2.", key)
        return key
    ...
    def key_mapping_mlp(key):
        return re.sub(r"^transformer.layers.(\d+).fc(1|2).", r"transformer.layers.\1.mlp.fc\2.", key)
    ...
    def key_mapping_attn(key):
        return re.sub(r"^transformer.layers.(\d+).self_attn.out_proj.", r"transformer.layers.\1.mixer.out_proj.", key)
...
def opt_config_to_gpt2_config(opt_config: OPTConfig) -> GPT2Config:
    ...
    word_embed_proj_dim = (
        None if opt_config.word_embed_proj_dim == opt_config.hidden_size else opt_config.word_embed_proj_dim
    )
    return GPT2Config(
