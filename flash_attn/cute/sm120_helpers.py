...
# SM120 (RTX 50) helpers using tcgen05 instructions.
# PTX references:
...
import flash_attn.cute.mma_sm120_desc as sm120_desc
...
def gemm_ptx(
    ...
    sB_layout = sB.layout
    idesc: int = const_expr(sm120_desc.mma_op_to_idesc(op))
    if const_expr(not is_ts):
        sA_swizzle = parse_swizzle_from_pointer(sA.iterator)
        smem_desc_base_a: int = const_expr(
            sm120_desc.make_smem_desc_base(
    ...
    if const_expr(not is_ts):
        smem_desc_start_a_lo = Int32(smem_desc_base_a_lo) | sm120_desc.make_smem_desc_start_addr(
            sA[None, None, 0].iterator
        )
    else:
        smem_desc_start_a_lo = None
    smem_desc_start_b_lo = Int32(smem_desc_base_b_lo) | sm120_desc.make_smem_desc_start_addr(
...
def gemm_ptx_loop(
    ...
    if const_expr(not is_ts):
        smem_desc_start_a_lo = Int32(
            smem_desc_base_a_lo | sm120_desc.make_smem_desc_start_addr(sA[None, None, 0].iterator)
        )
    else:
        smem_desc_start_a_lo = None
    smem_desc_start_b_lo = Int32(
        smem_desc_base_b_lo | sm120_desc.make_smem_desc_start_addr(sB[None, None, 0].iterator)
    )
