...
def check_tensor_for_nans(tensor, name="tensor"):
    ...
    if isinstance(tensor, torch.Tensor):
        ...
        try:
            if hasattr(tensor, "_dlpack_data"):
                ...
                print(
                    f"Warning: {name} is a CuTe tensor but cannot access DLPack data. "
    ...
    if has_nan:
        ...
        if torch_tensor.numel() > 0:
            ...
            if finite_mask.any():
                print(
                    f"  Finite values - Min: {torch_tensor[finite_mask].min().item():.6f}, "
...
def create_scale_factor_tensor(
    ...
    init_config = cutlass_torch.ScalarInitConfig(value=sf_value)
    ...
    ref_f32_torch_tensor_cpu = (
        ref_f32_torch_tensor_cpu.permute(
            2, 3, 0, 1
        )  # (mn, sf_k, batch, nheads) -> (batch, nheads, mn, sf_k)
        .unsqueeze(-1)
        .expand(batch, nheads, mn, sf_k, sf_vec_size)
        .reshape(batch, nheads, mn, sf_k * sf_vec_size)
        .permute(
            2, 3, 0, 1
        )  # (batch, nheads, mn, sf_k * sf_vec_size) -> (mn, sf_k * sf_vec_size, batch, nheads)
...
def create_blockscaled_attention_tensors(
    batch,
    seqlen_q,
    seqlen_k,
    nheads,
    ...
    if debug:
        q_ref = torch.full(
            (batch, seqlen_q, nheads, headdim), fill_value=1.0, device=device, dtype=torch.float32
        )
        k_ref = torch.full(
            (batch, seqlen_k, nheads_kv, headdim),
            fill_value=1.0,
            device=device,
            dtype=torch.float32,
        )
        ...
        v_ref = torch.zeros(
            (batch, seqlen_k, nheads_kv, headdim_v), device=device, dtype=torch.float32
        )
    ...
    q_tensor = cutlass_torch.convert_cute_tensor(q_ref, q_tensor, ab_dtype, is_dynamic_layout=True)
    k_tensor = cutlass_torch.convert_cute_tensor(k_ref, k_tensor, ab_dtype, is_dynamic_layout=True)
    ...
    q_sf_ref, q_sf_tensor, q_sf_torch_underlying = create_scale_factor_tensor(
        batch, seqlen_q, nheads, headdim, sf_vec_size, sf_dtype, ab_dtype, device, debug=debug
    )
    ...
    if return_torch:
        ...
        return (
            ...
            k_torch_underlying,
            v_torch_underlying,
            q_sf_torch_underlying,
            k_sf_torch_underlying,
            v_sf_torch_underlying,
            q_ref,
            k_ref,
            v_ref,
        )
...
def main(
    ...
    for i, (batch_size, seqlen, nheads, headdim) in enumerate(configs):
        ...
        try:
            ...
            print(
                f"FP4 Attention fwd: {m_fp4.mean * 1e3:.3f}ms, {(nFLOPS / m_fp4.mean * 1e-12):.1f} TFLOPS"
            )
        ...
        try:
            ...
            print(
                f"Reference fwd: {m_ref.mean * 1e3:.3f}ms, {(nFLOPS / m_ref.mean * 1e-12):.1f} TFLOPS"
            )
        ...
        try:
            ...
            for mb in range(min(seqlen_q // m_block_size_test, 4)):
                ...
                stage = mb % 2
                print(
                    f"    m_block={mb} stage={stage}: force_fp4={force_fp4_t[0, s, 0, 0].item():.4f} ref={ref_t[0, s, 0, 0].item():.4f}"
                )
        except Exception as e:
        ...
        if fp4_out is not None and ref_out is not None:
            try:
                ...
                for mb in range(min(seqlen_q // m_block_size, 8)):
                    ...
                    stage = mb % 2
                    print(
                        f"  m_block={mb} stage={stage}: fp4[0,{s},0,0]={fp4_t[0, s, 0, 0].item():.4f} ref={ref_t[0, s, 0, 0].item():.4f}"
                    )
                import traceback
...
if __name__ == "__main__":
    ...
    parser.add_argument("--quant_v", action="store_true", help="Deprecated alias for --pv_mode=fp4")
    parser.add_argument(
        "--qk_mode",
        choices=["nvfp4", "mxfp8"],
        default="nvfp4",
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Debug precision, set all tensors to 1.0"
    )
