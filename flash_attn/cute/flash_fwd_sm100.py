...
class FlashAttentionForwardSm100:
    ...
    def __call__(
        ...
        if const_expr(self.block_scaled_qk and mSFQ is not None):
            ...
            # TODO: SF TMA bytes will be added when SF loading is fixed
            pass
        ...
        sfq_smem_size = cute.cosize(sSFQ_layout) if const_expr(self.block_scaled_qk) else 1
        sfk_smem_size = cute.cosize(sSFK_layout) if const_expr(self.block_scaled_qk) else 1
    ...
    def kernel(
        ...
        if warp_idx >= self.load_warp_ids[0] and warp_idx <= self.load_warp_ids[-1]:
            ...
            self.load(
                ...
                sSFK=sSFK if const_expr(self.block_scaled_qk) else None,
            )
        ...
        if warp_idx == self.mma_warp_id:
            ...
            if const_expr(self.block_scaled_qk and sSFQ is not None):
                ...
                sfq_tmem_ptrs_f32 = [
                    cute.make_ptr(
                        Float32,
                        self.tmem_s_offset[sfq_stage_order[stage]],
    ...
    def load(
        ...
        while work_tile.is_valid_tile:
            ...
            # TODO: SF TMA loading — cpasync.tma_partition shape mismatch
            # The SMEM layout from make_smem_layout_sfb produces nested modes
            # that don't match the TMA atom's expected flat partition.
            # Need to investigate: use a separate flat SMEM layout for TMA,
            # or port the FP4 kernel's exact SharedStorage/get_tensor approach.
            pass
            if const_expr(self.use_tma_Q):
