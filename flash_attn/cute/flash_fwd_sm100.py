...
from flash_attn.cute.logging import fa_log, fa_printf
...
class FlashAttentionForwardSm100:
    ...
    def __init__(
        ...
        # NB: check CLC first since it's opt-in and we don't want other schedulers to override it
        ...
        fa_log(1, f"use_clc_scheduler={self.use_clc_scheduler}, TileScheduler={self.TileScheduler.__name__}, USE_2CTA={self.use_2cta_instrs}")
    ...
    def __call__(
        ...
        if const_expr(self.use_clc_scheduler):
            tile_sched_params = TileScheduler.to_underlying_arguments(
                tile_sched_args, sched_stages=self.sched_stages
            )
        else:
