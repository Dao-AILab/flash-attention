...
def get_num_sms(device_index: int) -> int:
    """Cache the host-visible SM count used by forward selection."""
...
def _flash_attn_fwd(
    ...
    *,
    ...
    heuristic_inputs = FwdHeuristicInputs(
        device_capacity=arch // 10,
        device_arch=arch,
        num_sms=132 if is_fake_mode() else get_num_sms(v.device.index),
        dtype=str(q_dtype),
        head_dim=head_dim,
    ...
    is_standard_kernel = qv is None and not use_dedicated_hd256_kernel
    is_standard_sm100 = is_standard_kernel and arch // 10 in [10, 11]
    main_spec = FwdMainKernelSpec(
        ...
        tile_m=tile_m if is_standard_kernel else None,
        tile_n=tile_n if is_standard_kernel else None,
        num_stages=num_stages if arch // 10 in [8, 9, 12] else None,
        num_threads=num_threads if arch // 10 in [8, 9, 12] else None,
        q_stage=q_stage if is_standard_sm100 else None,
        is_split_kv=is_split_kv if is_standard_sm100 else None,
        pack_gqa=pack_gqa,
        arch=arch,
        paged_kv_non_tma=(
            page_size not in [None, tile_n]
            if kernel_family in ("sm9", "sm10", "sm11", "mla_sm100")
            else None
        ),
        use_2cta_instrs=use_2cta_instrs if is_standard_sm100 else None,
        q_subtile_factor=(
            q_subtile_factor if kernel_family in ("sm9", "sm10", "sm11") else None
        ),
        kv_subtile_factor=kv_subtile_factor if is_standard_sm100 else None,
        mma_pv_is_rs=mma_pv_is_rs if kernel_family == "sm9" else None,
        intra_wg_overlap=intra_wg_overlap if kernel_family == "sm9" else None,
        use_clc_scheduler=use_clc_scheduler if is_standard_sm100 else None,
        is_persistent=is_persistent if is_standard_sm100 else None,
        use_tma_o=use_tma_o if is_standard_sm100 else None,
        has_q=q is not None,
        has_qv=qv is not None,
        has_p=p is not None,
...
def _flash_attn_fwd_combine(
    ...
    combine_spec = FwdCombineKernelSpec(
        ...
        has_lse=lse is not None,
        has_varlen_batch_idx=varlen_batch_idx is not None,
    )
