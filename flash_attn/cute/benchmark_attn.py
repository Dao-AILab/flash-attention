...
from flash_attn.cute.interface import flash_attn_func, flash_attn_varlen_func, get_scheduler_metadata
    ...
    if causal:
        ...
        if window_size == (None, None):
            ...
            col_left = torch.maximum(
                ...
                + seqlen_k
                - seqlen_q
                - (window_size[0] if window_size[0] is not None else 0),
                torch.tensor(0),
            )
            col_right = torch.minimum(
                ...
                + seqlen_k
                - seqlen_q
                + (window_size[1] if window_size[1] is not None else seqlen_k - 1),
...
def bandwidth_fwd(batch, nheads, nheads_kv, seqlen_q, seqlen_k, headdim, headdim_v, dtype, varlen=False, total_q=None, total_k=None):
    """
    Calculate memory bandwidth for forward pass in bytes.
    For varlen, pass total_q and total_k (actual total tokens from cu_seqlens).
    """
    bytes_per_element = dtype.itemsize
    if varlen:
        assert total_q is not None and total_k is not None, "For varlen, must provide total_q and total_k"
        memory_accessed = (
            total_q * nheads * headdim * bytes_per_element  # Q read
            + total_k * nheads_kv * headdim * bytes_per_element  # K read
            + total_k * nheads_kv * headdim_v * bytes_per_element  # V read
            + total_q * nheads * headdim_v * bytes_per_element  # O write
        )
    else:
        memory_accessed = (
            batch * seqlen_q * nheads * headdim * bytes_per_element  # Q read
            + batch * seqlen_k * nheads_kv * headdim * bytes_per_element  # K read
            + batch * seqlen_k * nheads_kv * headdim_v * bytes_per_element  # V read
            + batch * seqlen_q * nheads * headdim_v * bytes_per_element  # O write
        )
    return memory_accessed


def bandwidth_bwd(batch, nheads, nheads_kv, seqlen_q, seqlen_k, headdim, headdim_v, dtype, varlen=False, total_q=None, total_k=None):
    """
    Calculate memory bandwidth for backward pass in bytes.
    For varlen, pass total_q and total_k (actual total tokens from cu_seqlens).
    """
    bytes_per_element = dtype.itemsize
    if varlen:
        assert total_q is not None and total_k is not None, "For varlen, must provide total_q and total_k"
        memory_accessed = (
            total_q * nheads * headdim * bytes_per_element * 2  # Q read + dQ write
            + total_k * nheads_kv * headdim * bytes_per_element * 2  # K read + dK write
            + total_k * nheads_kv * headdim_v * bytes_per_element * 2  # V read + dV write
            + total_q * nheads * headdim_v * bytes_per_element  # dO read
            + total_q * nheads * headdim_v * bytes_per_element  # O read (for softmax)
        )
    else:
        memory_accessed = (
            batch * seqlen_q * nheads * headdim * bytes_per_element * 2  # Q read + dQ write
            + batch * seqlen_k * nheads_kv * headdim * bytes_per_element * 2  # K read + dK write
            + batch * seqlen_k * nheads_kv * headdim_v * bytes_per_element * 2  # V read + dV write
            + batch * seqlen_q * nheads * headdim_v * bytes_per_element  # dO read
            + batch * seqlen_q * nheads * headdim_v * bytes_per_element  # O read (for softmax)
        )
    return memory_accessed
...
bs_seqlen_vals = [(1, 256 * 1024)]
time_f = {}
time_b = {}
splits_vals = [1, 2, 3, 4, 5, 6, 7, 8, 16, 32]
# splits_vals = [1]
...
for headdim in [128]:
    ...
    for batch_size, seqlen in bs_seqlen_vals:
        for num_splits in splits_vals:
            ...
            v_cute = v if not V_colmajor else v_colmajor
            qv = (
            ...
            for causal in [False, True]:
                # for causal in [True]:
                print(f"\n### {headdim = }, {causal = }, {seqlen = }, {num_splits = } ###")
                nFLOPS = flops(
                    batch_size,
                    nheads,
                    seqlen_q,
                ...
                if headdim <= 256 and dtype != torch.float8_e4m3fn and headdim == headdim_v:
                    if triton_attention is not None:
                        qt, kt, vt = [
                            x.detach().transpose(1, 2).contiguous().requires_grad_() for x in [q, k, v]
                        ]
                        time.sleep(
                            1
                ...
                if not varlen:
                    m1 = time_fwd(
                        ...
                        v_cute if page_size is None else v_paged,
                    ...
                    # pytorch_profiler(flash_attn_func, q, k if page_size is None else k_paged, v_cute if page_size is None else v_paged, page_table=page_table, causal=causal, window_size=window_size, softcap=softcap, num_splits=num_splits, pack_gqa=pack_gqa)
                else:
                    scheduler_metadata = None
                    if precompute_metadata:
                        print(f"### precompute_metadata: {precompute_metadata} ###")
                        scheduler_metadata = get_scheduler_metadata(
                            ...
                            pack_gqa=pack_gqa if pack_gqa is not None else False,
                            is_causal=causal,
                            enable_pdl=False,
                            sort=False,
                            seqlen_k_new=0,
                        ...
                        _, num_splits_dynamic, _, _, _ = scheduler_metadata
                        print(f"num_splits_dynamic: {num_splits_dynamic}")
                    m1 = time_fwd(
                    ...
                    # pytorch_profiler(flash_attn_varlen_func, q_unpad, k_unpad, v_unpad, cu_seqlens_q, cu_seqlens_k, causal=causal, window_size=window_size, softcap=softcap, num_splits=num_splits)
                time_f[(causal, headdim, batch_size, seqlen), "CuTe"] = m1.mean
                if dtype != torch.float8_e4m3fn and headdim == headdim_v and not DISABLE_BACKWARD:
                    ...
                    time_b[(causal, headdim, batch_size, seqlen), "CuTe"] = m1b.mean
                    # time.sleep(1)
                    # if not varlen:
                    #     pytorch_profiler(flash_attn_func, q, k, v, causal=causal, deterministic=deterministic, backward=True)
                    # else:
                    #     pytorch_profiler(flash_attn_varlen_func, q_unpad, k_unpad, v_unpad, cu_seqlens_q, cu_seqlens_k, seqlen_q, seqlen, causal=causal, deterministic=deterministic, backward=True)
                # benchmark_forward(torch.clone, k, repeats=repeats, verbose=verbose, desc='Memcpy')

                # Calculate bandwidth - use actual total tokens for varlen
                if varlen:
                    total_q_actual = int(cu_seqlens_q[-1].item())
                    total_k_actual = int(cu_seqlens_k[-1].item())
                    nBytes_fwd = bandwidth_fwd(
                        batch_size, nheads, nheads_kv, seqlen_q, seqlen, headdim, headdim_v, dtype, varlen=True,
                        total_q=total_q_actual, total_k=total_k_actual
                    )
                    nBytes_bwd = bandwidth_bwd(
                        batch_size, nheads, nheads_kv, seqlen_q, seqlen, headdim, headdim_v, dtype, varlen=True,
                        total_q=total_q_actual, total_k=total_k_actual
                    )
                else:
                    nBytes_fwd = bandwidth_fwd(
                        batch_size, nheads, nheads_kv, seqlen_q, seqlen, headdim, headdim_v, dtype, varlen=False
                    )
                    nBytes_bwd = bandwidth_bwd(
                        batch_size, nheads, nheads_kv, seqlen_q, seqlen, headdim, headdim_v, dtype, varlen=False
                    )
                if headdim <= 256 and dtype != torch.float8_e4m3fn and headdim == headdim_v:
                    if triton_attention is not None:
                        bandwidth_gb_s = nBytes_fwd / m3.mean / 1e9
                        print(
                            f"Triton fwd: {m3.mean * 1e3:.3f}ms, {(nFLOPS / m3.mean * 1e-12):.1f} TFLOPS, {bandwidth_gb_s:.2f} GB/s"
                        )
                        # if causal:
                        #     print(f'Triton bwd: {m3b.mean * 1e3:.3f}ms, {(2.5 * nFLOPS / m3b.mean * 1e-12):.1f} TFLOPS')
                    if cudnn is not None:
                        bandwidth_gb_s_fwd = nBytes_fwd / m2.mean / 1e9
                        bandwidth_gb_s_bwd = nBytes_bwd / m2b.mean / 1e9
                        print(
                            f"CuDNN fwd: {m2.mean * 1e3:.3f}ms, {(nFLOPS / m2.mean * 1e-12):.1f} TFLOPS, {bandwidth_gb_s_fwd:.2f} GB/s"
                        )

                        print(
                            f"CuDNN bwd: {m2b.mean * 1e3:.3f}ms, {(2.5 * nFLOPS / m2b.mean * 1e-12):.1f} TFLOPS, {bandwidth_gb_s_bwd:.2f} GB/s"
                        )
                bandwidth_gb_s_fwd = nBytes_fwd / m1.mean / 1e9
                print(
                    f"CuTe fwd: {m1.mean * 1e3:.3f}ms, {(nFLOPS / m1.mean * 1e-12):.1f} TFLOPS, {bandwidth_gb_s_fwd:.2f} GB/s"
                )
                if dtype != torch.float8_e4m3fn and headdim == headdim_v and not DISABLE_BACKWARD:
                    bandwidth_gb_s_bwd = nBytes_bwd / m1b.mean / 1e9
                    print(
                        f"CuTe bwd: {m1b.mean * 1e3:.3f}ms, {(2.5 * nFLOPS / m1b.mean * 1e-12):.1f} TFLOPS, {bandwidth_gb_s_bwd:.2f} GB/s"
                    )
    ...
    # import pickle
    # # with open(f'cute_attn_time_h100_hdim{headdim}_causal.plk', 'wb') as fp:
    # # with open(f'cute_attn_time_h100_cudnn_triton_20241208.plk', 'wb') as fp:
    # with open(f'cute_attn_time_h100_20250313.plk', 'wb') as fp:
    # # with open(f'cute_attn_time_h100_fp8_hdim{headdim}.plk', 'wb') as fp:
    # # with open(f'cute_attn_time_h100_hdim{headdim}_1031.plk', 'wb') as fp:
