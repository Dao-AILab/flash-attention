    ...
    def kernel(
        ...
        if work_tile.is_valid_tile:
            ...
            if cutlass.const_expr(not seqlen.has_cu_seqlens_q):
                ...
                mdPsum_cur = cute.domain_offset((seqlen.offset_q_padded,), mdPsum[num_head, None])
                headdim_v = mO.shape[2]
            ...
            if cutlass.const_expr(mdQaccum is not None):
                if cutlass.const_expr(not seqlen.has_cu_seqlens_q):
                    ...
                    mdQaccum_cur = cute.domain_offset(
                        (seqlen.offset_q_padded * self.head_dim_padded,), mdQaccum[num_head, None]
                    )
            ...
            if cutlass.const_expr(mLSE is not None):
                if cutlass.const_expr(not seqlen.has_cu_seqlens_q):
                    ...
                    mLSElog2_cur = cute.domain_offset((seqlen.offset_q_padded,), mLSElog2[num_head, None])
