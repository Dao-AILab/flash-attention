...
from flash_attn.cute.logging import fa_printf
...
class TileSchedulerProtocol(Protocol):
    ...
    The `mbarrier_addr` parameter in `advance_to_next_work` enables CLC support:
    - Static schedulers: assert mbarrier_addr is None, use grid-stride iteration
    - CLC schedulers: assert mbarrier_addr is not None, issue async CLC query
    """
    ...
    def advance_to_next_work(self, *, mbarrier_addr=None) -> None:
        ...
        Args:
            mbarrier_addr: For CLC schedulers, the mbarrier address for async query.
                          Static schedulers should assert this is None.
        """
        ...
...
class CLCDynamicTileScheduler:
    ...
    - Scheduler warp (producer): Issues CLC queries via advance_to_next_work(mbarrier_addr=...)
    - All other warps (consumers): Read work tiles via get_current_work()

    Requires `use_tma_KV=True` so the kernel can dedicate an empty warp to drive CLC queries.
    """
    ...
    class Params(ParamsBase):
        ...
        def create(
            ...
            *,
            ...
            assert sched_stages == 1, f"CLC scheduler only supports 1 stage, got {sched_stages}"
            num_batch_splits = (
                args.num_batch * args.num_splits if const_expr(args.is_split_kv) else args.num_batch
            )
            return CLCDynamicTileScheduler.Params(
                num_block=args.num_block,
                num_head=args.num_head,
                num_batch_splits=num_batch_splits,
    ...
    def to_underlying_arguments(
        ...
        *,
        ...
        return CLCDynamicTileScheduler.Params.create(args, sched_stages, loc=loc, ip=ip)
    ...
    def get_grid_shape(
        ...
        *,
        ...
        # CLC needs the full problem grid as backlog for try_cancel work-stealing.
        # Grid x must be a multiple of cluster_shape_m for CUDA cluster launch.
        return (
            cute.round_up(params.num_block, params.cluster_shape_m),
            params.num_head,
            params.num_batch_splits,
    ...
    @cute.jit
    def _work_to_coords(
        self, block_idx: Int32, head_idx: Int32, batch_split_idx: Int32, is_valid
    ) -> WorkTileInfo:
        """Convert CUTLASS work tile coordinates to WorkTileInfo."""
        if const_expr(self.params.cluster_shape_m > 1):
        ...
        split_idx = Int32(0)
        if const_expr(self.params.is_split_kv):
            batch_idx, split_idx = divmod(batch_split_idx, self.params.num_splits_divmod)
        else:
    ...
    def get_current_work(self, *, loc=None, ip=None) -> WorkTileInfo:
        ...
        result = self._work_to_coords(
            work.tile_idx[0], work.tile_idx[1], work.tile_idx[2], work.is_valid_tile
        )
        if cute.arch.thread_idx()[0] == 0:
            self._debug_print("pull", *result.tile_idx, result.is_valid_tile)
        return result
    ...
    def initial_work_tile_info(self, *, loc=None, ip=None) -> WorkTileInfo:
        ...
        if cute.arch.thread_idx()[0] == 0:
            self._debug_print("map", *result.tile_idx, result.is_valid_tile)
        return result
    ...
    def _debug_print(self, phase: str, block_idx, head_idx, batch_idx, split_idx, is_valid):
        linear_idx = (
            batch_idx * self.params.num_head * self.params.num_block
            + head_idx * self.params.num_block
            + block_idx
        )
        total_tiles = self.params.num_block * self.params.num_head * self.params.num_batch_splits
        fa_printf(
            3,
