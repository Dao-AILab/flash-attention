...
@dataclass(frozen=True)
class FwdMainKernelConfigSpec:
    """Code-generation projection of a resolved forward config."""
    ...
    def project(
        ...
        *,
        ...
        """Keep only config values consumed by the selected kernel family."""
        is_standard = kernel_family in ("sm8", "sm9", "sm10", "sm11", "sm12")
        is_standard_sm100 = kernel_family in ("sm10", "sm11")
        return cls(
            ...
            q_stage=config.q_stage if is_standard_sm100 else None,
            is_split_kv=config.num_splits > 1 if is_standard_sm100 else None,
            pack_gqa=pack_gqa,
            arch=arch,
            paged_kv_non_tma=(
            ...
            use_2cta_instrs=config.use_2cta_instrs if is_standard_sm100 else None,
            q_subtile_factor=(
            ...
            mma_pv_is_rs=config.mma_pv_is_rs if kernel_family == "sm9" else None,
            intra_wg_overlap=config.intra_wg_overlap if kernel_family == "sm9" else None,
            use_clc_scheduler=config.use_clc_scheduler if is_standard_sm100 else None,
            is_persistent=config.is_persistent if is_standard_sm100 else None,
            use_tma_o=config.use_tma_o if is_standard_sm100 else None,
        )
...
class FwdMainKernelSpec:
    ...
    config: FwdMainKernelConfigSpec
    has_q: bool
    has_qv: bool
    has_p: bool
    has_row_max: bool
