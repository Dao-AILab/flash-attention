...
class AttentionMask:
    ...
    def apply_mask(
        ...
        if const_expr(not mask_causal and not mask_local and mask_mod is None):
            ...
            mask_mod_vec_size_value = getattr(mask_mod, "__vec_size__", None)
            mask_mod_is_vectorized = const_expr(mask_mod_vec_size_value is not None)
            mask_mod_vec_size = const_expr(
                1 if mask_mod_vec_size_value is None else mask_mod_vec_size_value
            )
            assert ncol % mask_mod_vec_size == 0
            ...
            for r in cutlass.range_constexpr(nrow):
                ...
                if const_expr(mask_mod_is_vectorized):
                    q_idx_ssa = utils.scalar_to_ssa(row_for_mod, cutlass.Int32).broadcast_to(
                        (mask_mod_vec_size,)
                    )
                    head_idx_ssa = utils.scalar_to_ssa(
                        head_idx_for_mod, cutlass.Int32
                    ).broadcast_to((mask_mod_vec_size,))
                    kv_idx_vec = cute.make_rmem_tensor(mask_mod_vec_size, cutlass.Int32)
                    global_col_idx_vec = cute.make_rmem_tensor(mask_mod_vec_size, cutlass.Int32)
                    for col in cutlass.range(0, ncol, mask_mod_vec_size, unroll_full=True):
                        for j in cutlass.range(mask_mod_vec_size, unroll_full=True):
                            col_idx_local = t0ScS_mn[0, col + j][COL]
                            global_col_idx = thr_col_offset + col_idx_local + n_block * self.tile_n
                            global_col_idx_vec[j] = global_col_idx
                            kv_idx_vec[j] = global_col_idx
                            if const_expr(wrap_aux_indices):
                                _, kv_idx_vec[j] = divmod(global_col_idx, fastdiv_mods[1])

                        mask_values = mask_mod(
                            batch_idx_ssa,
                            head_idx_ssa,
                            q_idx_ssa,
                            kv_idx_vec.load(),
                        ...
                        for j in cutlass.range(mask_mod_vec_size, unroll_full=True):
                            global_col_idx = global_col_idx_vec[j]
                            cond = cutlass.Boolean(mask_values[j])
                            if const_expr(mask_seqlen):
                                out_of_bounds = (row_for_seqlen >= self.seqlen_q) or (
                                    global_col_idx >= self.seqlen_k
                                )
                                if out_of_bounds:
                                    acc_S_mn[r, col + j] = -cutlass.Float32.inf
                                else:
                                    acc_S_mn[r, col + j] = (
                                        acc_S_mn[r, col + j] if cond else -cutlass.Float32.inf
                                    )
                            else:
                                acc_S_mn[r, col + j] = (
                                    acc_S_mn[r, col + j] if cond else -cutlass.Float32.inf
                    ...
                    for col in cutlass.range_constexpr(ncol):
                        ...
                        mask_value = mask_mod(
                            ...
                            utils.scalar_to_ssa(head_idx_for_mod, cutlass.Int32),
                            utils.scalar_to_ssa(row_for_mod, cutlass.Int32),
                            kv_idx_ssa,
                            self.seqlen_info,
                            aux_tensors,
                        )
    ...
    def apply_mask_sm100(
        ...
        ROW = 1 if const_expr(self.swap_AB) else 0
        COL = 0 if const_expr(self.swap_AB) else 1
        ...
        if const_expr(rBitmask is not None):
            ...
            assert ncol % mask_mod_vec_size == 0
            if const_expr(not mask_mod_is_vectorized):
                batch_idx_ssa = utils.scalar_to_ssa(batch_idx, cutlass.Int32)
                for i in cutlass.range_constexpr(ncol):
                    global_row = tScS_t2r[i][ROW] + m_block * self.tile_m
                    global_col = tScS_t2r[i][COL] + n_block * self.tile_n
                ...
                global_row_vec = cute.make_rmem_tensor(mask_mod_vec_size, cutlass.Int32)
                mask_row_vec = cute.make_rmem_tensor(mask_mod_vec_size, cutlass.Int32)
                mask_row_for_mod_vec = cute.make_rmem_tensor(mask_mod_vec_size, cutlass.Int32)
                global_col_vec = cute.make_rmem_tensor(mask_mod_vec_size, cutlass.Int32)
                global_col_for_mod_vec = cute.make_rmem_tensor(mask_mod_vec_size, cutlass.Int32)
                head_idx_vec = cute.make_rmem_tensor(mask_mod_vec_size, cutlass.Int32)
                head_offset_vec = cute.make_rmem_tensor(mask_mod_vec_size, cutlass.Int32)
                for chunk in cutlass.range_constexpr(ncol // mask_mod_vec_size):
                    base = chunk * mask_mod_vec_size
                    for j in cutlass.range_constexpr(mask_mod_vec_size):
                        global_row_vec[j] = tScS_t2r[base + j][ROW] + m_block * self.tile_m
                        global_col_vec[j] = tScS_t2r[base + j][COL] + n_block * self.tile_n
                        global_col_for_mod_vec[j] = global_col_vec[j]

                        if const_expr(self.qhead_per_kvhead_packgqa != 1):
                            assert head_divmod is not None
                            mask_row_vec[j], head_offset_vec[j] = divmod(
                                global_row_vec[j], head_divmod
                            )
                            head_idx_vec[j] = (
                                head_idx * self.qhead_per_kvhead_packgqa + head_offset_vec[j]
                            )
                        else:
                            head_idx_vec[j] = head_idx
                            mask_row_vec[j] = global_row_vec[j]

                        mask_row_for_mod_vec[j] = mask_row_vec[j]
                        if const_expr(has_fastdiv and aux_tensors is not None):
                            if check_q_boundary:
                                _, mask_row_for_mod_vec[j] = divmod(
                                    mask_row_vec[j], fastdiv_mods[0]
                                )
                        if const_expr(has_fastdiv and mask_seqlen and aux_tensors is not None):
                            _, global_col_for_mod_vec[j] = divmod(
                                global_col_vec[j], fastdiv_mods[1]
                            )

                    mask_values = mask_mod(
                        batch_idx_ssa,
                        head_idx_vec.load(),
                        mask_row_for_mod_vec.load(),
                        global_col_for_mod_vec.load(),
                        self.seqlen_info,
                        aux_tensors,
                    )
                    for j in cutlass.range_constexpr(mask_mod_vec_size):
                        cond = cutlass.Boolean(mask_values[j])
                        acc_S[base + j] = acc_S[base + j] if cond else -Float32.inf
                        if const_expr(mask_seqlen):
                            acc_S[base + j] = (
                                -Float32.inf
                                if global_col_vec[j] >= self.seqlen_k
                                else acc_S[base + j]
                            )
                        if check_q_boundary:
                            acc_S[base + j] = (
                                -Float32.inf
                                if mask_row_vec[j] >= self.seqlen_q
                                else acc_S[base + j]
                            )
        ...
        if const_expr(not mask_causal and not mask_local and mask_mod is not None):
            ...
            if is_full_block:
                ...
                if const_expr(mask_mod_is_vectorized):
                    ...
                    global_q_vec = cute.make_rmem_tensor(mask_mod_vec_size, cutlass.Int32)
                    global_kv_vec = cute.make_rmem_tensor(mask_mod_vec_size, cutlass.Int32)
                    q_idx_vec = cute.make_rmem_tensor(mask_mod_vec_size, cutlass.Int32)
                    kv_idx_vec = cute.make_rmem_tensor(mask_mod_vec_size, cutlass.Int32)
                    for chunk in cutlass.range_constexpr(ncol // mask_mod_vec_size):
                        base = chunk * mask_mod_vec_size
                        for j in cutlass.range_constexpr(mask_mod_vec_size):
                            global_q_vec[j] = tScS_t2r[base + j][ROW] + m_block * self.tile_m
                            global_kv_vec[j] = tScS_t2r[base + j][COL] + n_block * self.tile_n
                            q_idx_vec[j] = global_q_vec[j]
                            kv_idx_vec[j] = global_kv_vec[j]
                            if const_expr(wrap_aux_indices):
                                _, q_idx_vec[j] = divmod(global_q_vec[j], fastdiv_mods[0])
                                _, kv_idx_vec[j] = divmod(global_kv_vec[j], fastdiv_mods[1])
                        ...
                        for j in cutlass.range_constexpr(mask_mod_vec_size):
                            cond = cutlass.Boolean(mask_values[j])
                            acc_S[base + j] = acc_S[base + j] if cond else -cutlass.Float32.inf
                            if const_expr(mask_seqlen):
                                q_out_of_bounds = check_m_boundary and (
                                    global_q_vec[j] >= self.seqlen_q
                                )
                                kv_out_of_bounds = global_kv_vec[j] >= self.seqlen_k
                                out_of_bounds = q_out_of_bounds or kv_out_of_bounds
                                acc_S[base + j] = (
                                    -cutlass.Float32.inf if out_of_bounds else acc_S[base + j]
                                )
                else:
                    batch_idx_ssa = utils.scalar_to_ssa(batch_idx, cutlass.Int32)
