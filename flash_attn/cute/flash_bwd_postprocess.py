...
class FlashAttentionBackwardSink:
    """Reduce dSink for direct-dQ backward kernels such as SM100 hd256."""

    def __init__(self, tile_m: int = 128, num_threads: int = 128):
    ...
    def __call__(
        ...
        if const_expr(mdPsum.element_type != cutlass.Float32):
            raise TypeError("dPsum must be Float32")
        if const_expr(mLSE.element_type != cutlass.Float32):
            raise TypeError("LSE must be Float32")
        if const_expr(mLearnableSink.element_type != cutlass.BFloat16):
        ...
        num_head = mQ.shape[1] if const_expr(mCuSeqlensQ is not None) else mQ.shape[2]
        ...
        self.kernel(
            ...
            smem=cute.size_in_bytes(cutlass.Float32, smem_layout),
            stream=stream,
        )
    ...
    def kernel(
        ...
        while batch_idx < num_batch:
            ...
            row = tidx
            while row < seqlen.seqlen_q:
                if const_expr(mCuSeqlensQ is None):
                    dpsum_val = mdPsum[batch_idx, head_idx, row]
                    lse_val = mLSE[batch_idx, head_idx, row]
                else:
                    dpsum_val = mdPsum[head_idx, seqlen.padded_offset_q + row]
                    lse_val = mLSE[head_idx, seqlen.offset_q + row]
        ...
        num_warps = self.num_threads // cute.arch.WARP_SIZE
        if lane_idx == 0:
            partials[warp_idx] = sink_sum
        cute.arch.barrier(barrier_id=1, number_of_threads=self.num_threads)
        if warp_idx == 0:
            sink_sum = partials[lane_idx] if lane_idx < num_warps else Float32(0.0)
            sink_sum = utils.warp_reduce(sink_sum, operator.add)
            if lane_idx == 0:
                mdSink[head_idx] = sink_sum.to(cutlass.BFloat16)
