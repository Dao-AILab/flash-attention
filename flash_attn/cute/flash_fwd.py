...
class FlashAttentionForwardSm80(FlashAttentionForwardBase):
    ...
    def __call__(
        ...
        mQ/mK/mV/mO has same data types(supports fp16 and bf16) and same layout:
        Fixed-length: (batch_size, seqlen_q, num_head, head_dim):(_, _, _, 1)
        Varlen: (total_q, num_head, head_dim):(_, _, 1)
        """
        assert learnable_sink is None, "Learnable sink is not supported in this kernel"
        assert mPageTable is None, "Paged KV not yet supported in SM80 kernel"
        self._check_type(
