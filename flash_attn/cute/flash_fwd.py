...
class FlashAttentionForwardBase:
    ...
    def epilogue(
        ...
        head_idx: Int32,
        batch_idx: Int32,
        entropy: Optional[cute.Tensor] = None,
        ...
        # Write entropy from rmem -> gmem (same layout as LSE)
        if const_expr(mEntropy is not None):
            if const_expr(not seqlen.has_cu_seqlens_q):
                mEntropy_cur = mEntropy[None, head_idx, batch_idx]
            else:
                offset = seqlen.offset_q if const_expr(not self.pack_gqa) else (0, seqlen.offset_q)
                mEntropy_cur = cute.domain_offset((offset,), mEntropy[None, head_idx])
            if const_expr(not self.pack_gqa):
                gEntropy = cute.local_tile(mEntropy_cur, (self.tile_m,), (m_block,))
                gEntropy_expanded_layout = cute.append(
                    gEntropy.layout, cute.make_layout((self.tile_hdimv,), stride=(0,))
                )
                gEntropy_expanded = cute.make_tensor(gEntropy.iterator, gEntropy_expanded_layout)
                thr_mma = tiled_mma.get_slice(tidx)
                taccOgEntropy = layout_utils.reshape_acc_to_mn(thr_mma.partition_C(gEntropy_expanded))
                taccOcO = layout_utils.reshape_acc_to_mn(thr_mma.partition_C(cO))
                t0accOcO = layout_utils.reshape_acc_to_mn(thr_mma.get_slice(0).partition_C(cO))
                if taccOcO[0][1] == 0:
                    for m in cutlass.range_constexpr(cute.size(taccOgEntropy.shape[1])):
                        if (
                            t0accOcO[m, 0][0]
                            < seqlen.seqlen_q - m_block * self.tile_m - taccOcO[0][0]
                        ):
                            taccOgEntropy[m, 0] = entropy[m]
            else:
                pack_gqa.store_LSE(mEntropy_cur, entropy, tiled_mma, tidx, m_block, seqlen.seqlen_q)
...
class FlashAttentionForwardSm80(FlashAttentionForwardBase):
    ...
    def __call__(
        ...
        if const_expr(mEntropy is not None):
            Entropy_layout_transpose = [2, 1, 0] if const_expr(mCuSeqlensQ is None) else [1, 0]
            mEntropy = cute.make_tensor(mEntropy.iterator, cute.select(mEntropy.layout, mode=Entropy_layout_transpose))
