...
template <bool Is_causal, bool Is_local, bool Has_alibi, bool Is_tree_attention=false>
struct Mask {

    const int max_seqlen_k, max_seqlen_q;
    const int window_size_left, window_size_right;
    const int sum_s_k, sum_s_q;
    const float alibi_slope;

    __forceinline__ __device__ Mask(const int max_seqlen_k, const int max_seqlen_q,
                                    const int window_size_left, const int window_size_right,
                                    const float alibi_slope=0.f, 
                                    const int sum_s_k = -1, const int sum_s_q = -1)
    ...
    __forceinline__ __device__ void apply_mask(Tensor<Engine, Layout> &tensor_,
        ...
        static constexpr bool Need_masking = Has_alibi || Causal_mask || Is_local || !Is_even_MN || Is_tree_attention;
        ...
        if constexpr (Need_masking) {
            ...
            static constexpr bool Col_idx_only = !(Has_alibi && !Is_causal) && !Is_local && !Causal_mask && !Is_tree_attention;
            const int lane_id = threadIdx.x % 32;
            const int col_idx_offset = col_idx_offset_ + (lane_id % 4) * 2;
