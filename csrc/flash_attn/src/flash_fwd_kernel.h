...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    Tensor tKgK = gmem_thr_copy_QK.partition_S(gK);  // (KCPY, KCPY_N, KCPY_K, nblocksN)
    ...
    flash::copy<Is_even_MN, Is_even_K>(gmem_tiled_copy_QK, tKgK(_, _, _, n_block), tKsK, tKcK, tKpK,
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        // Advance gV
        if (masking_step > 0) {
            flash::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_V, tVgV(_, _, _, n_block), tVsV, tVcV, tVpV);
        } else {
        ...
        if (n_block > n_block_min) {
            flash::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QK, tKgK(_, _, _, n_block - 1), tKsK, tKcK, tKpK);
...
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    if constexpr (Append_KV) {
        ...
        Tensor tVgVnew = gmem_thr_copy_V.partition_S(gVnew);  // (VCPY, VCPY_N, VCPY_K)
    ...
    int n_block = n_block_max - 1;
    // We don't need to clear the sK smem tiles since we'll mask out the scores anyway.
    flash::copy<Is_even_MN, Is_even_K>(gmem_tiled_copy_QK, tKgK, tKsK, tKcK, tKpK,
                                       binfo.actual_seqlen_k - n_block * kBlockN);
    cute::cp_async_fence();
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (n_block > n_block_min) {
            ...
            flash::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QK, tKgK, tKsK, tKcK, tKpK);
            // This cp_async_fence needs to be in the if block, otherwise the synchronization
            // isn't right and we get race conditions.
            cute::cp_async_fence();
        }
