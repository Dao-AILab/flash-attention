...
// Include torch/extension.h first to ensure correct macro definitions and
// prevent issues with undefined symbols
...
// Helper function to unpack PhiloxCudaState since at::cuda::philox::unpack is
// not visible
inline std::tuple<uint64_t, uint64_t> unpack_philox(at::PhiloxCudaState arg) {
  if (arg.captured_) {
...
void set_params_fprop(
    ...
    void *softmax_lse_d, double p_dropout, double softmax_scale,
...
void set_params_dgrad(
    ...
    void *dsoftmax_sum_d, double p_dropout, double softmax_scale,
    int64_t window_size_left, int64_t window_size_right, const double softcap,
    bool deterministic, const bool unpadded_lse) {
...
set_params_splitkv(Flash_fwd_params &params, const int64_t batch_size,
                   const int64_t num_heads, const int64_t head_size,
                   const int64_t max_seqlen_k, const int64_t max_seqlen_q,
                   const int64_t head_size_rounded, const double p_dropout,
                   const int64_t num_splits, const int64_t num_sm,
...
std::vector<at::Tensor> mha_fwd(
  ...
  TORCH_CHECK(head_size % 8 == 0, "query, key, value, and out_ must have a "
                                  "head_size that is a multiple of 8");
  TORCH_CHECK(
...
std::vector<at::Tensor> mha_varlen_fwd(
    ...
    const at::Tensor &v, // total_k x num_heads_k x head_size, total_k :=
                         ...
                         // num_heads_k x head_size if there's a block_table.
    std::optional<at::Tensor>
        &out_, // total_q x num_heads x head_size, total_q := \sum_{i=0}^{b} s_i
    const at::Tensor &cu_seqlens_q, // b+1
    const at::Tensor &cu_seqlens_k, // b+1
    std::optional<at::Tensor>
        &seqused_k, // b. If given, only this many elements of each batch
                    // element's keys are used.
    std::optional<at::Tensor> &leftpad_k_, // batch_size
    std::optional<at::Tensor>
...
std::vector<at::Tensor> mha_bwd(
    const at::Tensor &dout, // batch_size x seqlen_q x num_heads, x
    ...
    const double softmax_scale, const bool is_causal, int64_t window_size_left,
    int64_t window_size_right, const double softcap, const bool deterministic,
    std::optional<at::Generator> gen_, std::optional<at::Tensor> &rng_state) {
  ...
  if (rng_state.has_value()) {
    ...
    std::lock_guard<std::mutex> lock(gen->mutex_);
    params.philox_args = gen->philox_cuda_state(counter_offset);
    auto [seed, offset] = unpack_philox(params.philox_args);
...
std::vector<at::Tensor> mha_varlen_bwd(
    ...
    const double p_dropout,     // probability to drop
    const double softmax_scale, const bool zero_tensors, const bool is_causal,
    int64_t window_size_left, int64_t window_size_right, const double softcap,
  ...
  if (loop) {
    ...
    // num_heads, head_size_rounded) because that would be too large if there is
    // a very long sequence and the rest of the sequences are short. Instead, we
    ...
    // apart. It's ok for us to do atomicAdds up to 128 rows beyond what we're
    // normally allowed to do. So we won't have to do any bound checking, and
    // performance should stay the same. Same holds for softmax_d, since LSE is
    // stored in unpadded format.
    if (!deterministic) {
      dq_accum = torch::empty(
  ...
  set_params_dgrad(
      params, batch_size, max_seqlen_q, max_seqlen_k, seqlen_q_rounded,
      seqlen_k_rounded, num_heads, num_heads_k, head_size, head_size_rounded, q,
      k, v, out, dout, dq, dk_expanded, dv_expanded, cu_seqlens_q.data_ptr(),
      cu_seqlens_k.data_ptr(), loop ? dq_accum.data_ptr() : nullptr, nullptr,
      nullptr, softmax_lse.data_ptr(), softmax_d.data_ptr(), p_dropout,
      softmax_scale, window_size_left, window_size_right, softcap,
      deterministic,
      /*unpadded_lse*/ true);
  params.dq_accum_split_stride = !deterministic ? 0 : dq_accum.stride(0);
  params.total_q = total_q;
...
std::vector<at::Tensor> mha_fwd_kvcache(
    ...
    std::optional<at::Tensor> &seqlens_k_,  // batch_size
  ...
  if (k_.has_value()) {
    ...
    TORCH_CHECK(seqlen_q <= seqlen_k, "If key is supplied, it must have seqlen "
                                      "<= the seqlen of the KV cache");
    k = k_.value();
    v = v_.value();
...
extern "C" {
// Dummy module init to allow "import flash_attn_2_cuda" to work and load the
// library
PyObject *PyInit_flash_attn_2_cuda(void) {
  static struct PyModuleDef module_def = {
      PyModuleDef_HEAD_INIT, "flash_attn_2_cuda", NULL, -1, NULL,
  };
  return PyModule_Create(&module_def);
}
}

TORCH_LIBRARY(flash_attn_2_cuda, m) {
  m.def("fwd(Tensor q, Tensor k, Tensor v, Tensor? out, Tensor? alibi_slopes, "
        "float p_dropout, float softmax_scale, bool is_causal, int "
        "window_size_left, int window_size_right, float softcap, bool "
        "return_softmax, Generator? gen) -> (Tensor, Tensor, Tensor, Tensor)");

  m.def("varlen_fwd(Tensor q, Tensor k, Tensor v, Tensor? out, Tensor "
        "cu_seqlens_q, Tensor cu_seqlens_k, Tensor? seqused_k, Tensor? "
        "leftpad_k, Tensor? block_table, Tensor? alibi_slopes, int "
        "max_seqlen_q, int max_seqlen_k, float p_dropout, float softmax_scale, "
        "bool zero_tensors, bool is_causal, int window_size_left, int "
        "window_size_right, float softcap, bool return_softmax, Generator? "
        "gen) -> (Tensor, Tensor, Tensor, Tensor)");

  m.def("bwd(Tensor dout, Tensor q, Tensor k, Tensor v, Tensor out, Tensor "
        "softmax_lse, Tensor? dq, Tensor? dk, Tensor? dv, Tensor? "
        "alibi_slopes, float p_dropout, float softmax_scale, bool is_causal, "
        "int window_size_left, int window_size_right, float softcap, bool "
        "deterministic, Generator? gen, Tensor? rng_state) -> (Tensor, Tensor, "
        "Tensor, Tensor)");

  m.def("varlen_bwd(Tensor dout, Tensor q, Tensor k, Tensor v, Tensor out, "
        "Tensor softmax_lse, Tensor? dq, Tensor? dk, Tensor? dv, Tensor "
        "cu_seqlens_q, Tensor cu_seqlens_k, Tensor? alibi_slopes, int "
        "max_seqlen_q, int max_seqlen_k, float p_dropout, float softmax_scale, "
        "bool zero_tensors, bool is_causal, int window_size_left, int "
        "window_size_right, float softcap, bool deterministic, Generator? gen, "
        "Tensor? rng_state) -> (Tensor, Tensor, Tensor, Tensor)");

  m.def(
      "fwd_kvcache(Tensor q, Tensor kcache, Tensor vcache, Tensor? k, Tensor? "
      "v, Tensor? seqlens_k, Tensor? rotary_cos, Tensor? rotary_sin, Tensor? "
      "cache_batch_idx, Tensor? leftpad_k, Tensor? block_table, Tensor? "
      "alibi_slopes, Tensor? out, float softmax_scale, bool is_causal, int "
      "window_size_left, int window_size_right, float softcap, bool "
      "is_rotary_interleaved, int num_splits) -> (Tensor, Tensor)");
}

TORCH_LIBRARY_IMPL(flash_attn_2_cuda, CUDA, m) {
